...
//	japp_randFix 0 == Same as basejka. Broken on Linux, fine on Windows
//	japp_randFix 1 == Use proper behaviour of RAND_MAX. Fine on Linux, fine on Windows
//	japp_randFix 2 == Intentionally break RAND_MAX. Broken on Linux, broken on Windows.
float RandFloat( float min, float max ) {
	int randActual = rand();
	float randMax = 32768.0f;
#if defined(_WIN32)
	if ( japp_randFix.integer == 2 )
...
void	G_DebugBoxLines( vector3 *mins, vector3 *maxs, int duration ) {
	vector3 start;
	vector3 end;
...
static int G_SaberAttackPower( gentity_t *ent, qboolean attacking ) {
	...
	if ( attacking ) { //the attacker gets a boost to help penetrate defense.
		...
		if ( ent->client->lastSaberStorageTime >= (level.time - 50) &&
			...
			while ( swingDist > 0 ) { //I would like to do something more clever. But I suppose this works, at least for now.
				baseLevel++;
	...
	if ( (ent->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)) ||
		(ent->client->ps.brokenLimbs & (1 << BROKENLIMB_LARM)) ) { //We're very weak when one of our arms is broken
		baseLevel *= 0.3f;
	}
	...
	if ( level.gametype == GT_POWERDUEL &&
		ent->client->sess.duelTeam == DUELTEAM_LONE ) { //get more power then
		return baseLevel * 2;
	}
	else if ( attacking && level.gametype == GT_SIEGE ) { //in siege, saber battles should be quicker and more biased toward the attacker
		return baseLevel * 3;
	}

	//	Com_Printf( "%i", baseLevel );
	return baseLevel;
}
...
void WP_ActivateSaber( gentity_t *self ) {
	...
	if ( self->NPC &&
		...
		(self->client->ps.forceHandExtendTime - level.time) > 200 ) { //if we're an NPC and in the middle of a taunt then stop it
		self->client->ps.forceHandExtend = HANDEXTEND_NONE;
		self->client->ps.forceHandExtendTime = 0;
	}
	else if ( self->client->ps.fd.forceGripCripple ) { //can't activate saber while being gripped
		return;
	}
...
void SaberUpdateSelf( gentity_t *ent ) {
	...
	if ( g_entities[ent->r.ownerNum].client->ps.saberInFlight && g_entities[ent->r.ownerNum].health > 0 ) { //let The Master take care of us now (we'll get treated like a missile until we return)
		ent->nextthink = level.time;
	...
	if ( g_entities[ent->r.ownerNum].client->ps.weapon != WP_SABER ||
		...
		(!g_entities[ent->r.ownerNum].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] && g_entities[ent->r.ownerNum].s.eType != ET_NPC) ) { //owner is not using saber, spectating, dead, saber holstered, or has no attack level
		ent->r.contents = 0;
		ent->clipmask = 0;
	}
	else { //Standard contents (saber is active)
#ifdef DEBUG_SABER_BOX
		if ( g_saberDebugBox.integer == 1 || g_saberDebugBox.integer == 4 ) {
			...
			G_DebugBoxLines( &dbgMins, &dbgMaxs, (10.0f / sv_fps.value) * 100 );
		}
#endif
		if ( ent->r.contents != CONTENTS_LIGHTSABER ) {
			if ( (level.time - g_entities[ent->r.ownerNum].client->lastSaberStorageTime) <= 200 ) { //Only go back to solid once we're sure our owner has updated recently
...
void WP_SaberInitBladeData( gentity_t *ent ) {
	...
	while ( i < level.num_entities ) { //make sure there are no other saber entities floating around that think they belong to this client.
...
static qboolean G_CheckLookTarget( gentity_t *ent, vector3 *lookAngles, float *lookingSpeed ) {
	...
	if ( ent->s.eType == ET_NPC &&
		ent->s.m_iVehicleNum &&
		ent->s.NPC_class != CLASS_VEHICLE ) { //an NPC bolted to a vehicle should just look around randomly
	...
	if ( ent->client->renderInfo.lookTarget >= 0 && ent->client->renderInfo.lookTarget < ENTITYNUM_WORLD ) {
		vector3	lookDir, lookOrg, eyeOrg;
		int i;

		if ( ent->client->renderInfo.lookMode == LM_ENT ) {
			gentity_t	*lookCent = &g_entities[ent->client->renderInfo.lookTarget];
			if ( lookCent ) {
				if ( lookCent != ent->enemy ) {//We turn heads faster than headbob speed, but not as fast as if watching an enemy
...
static void G_G2NPCAngles( gentity_t *ent, vector3 legs[3], vector3 *angles ) {
	...
	if ( ent->client ) {
		if ( (ent->client->NPC_class == CLASS_PROBE)
			...
			vector3	trailingLegsAngles;

			if ( ent->s.eType == ET_NPC &&
				ent->s.m_iVehicleNum &&
				ent->s.NPC_class != CLASS_VEHICLE ) { //an NPC bolted to a vehicle should use the full angles
			...
			*/
			...
			if ( ent->client->NPC_class == CLASS_ATST ) {//body pitch
			...
			if ( ent->client->renderInfo.lookingDebounceTime > level.time ) {	//adjust for current body orientation
				vector3	oldLookAngles;
...
static void G_G2PlayerAngles( gentity_t *ent, vector3 legs[3], vector3 *legsAngles ) {
	...
	vector3 turAngles;
	vector3 lerpOrg, lerpAng;

	if ( ent->s.eType == ET_NPC && ent->client ) { //sort of hacky, but it saves a pretty big load off the server
		int i = 0;
		gentity_t *clEnt;
		...
		while ( i < MAX_CLIENTS ) {
			...
			if ( clEnt && clEnt->inuse && clEnt->client &&
				trap->InPVS( &clEnt->client->ps.origin, &ent->client->ps.origin ) ) { //this client can see him
				break;
			}
		...
		if ( i == MAX_CLIENTS ) { //no one can see him, just return
			return;
		}
	}
	...
	if ( ent->localAnimIndex <= 1 ) { //don't do these things on non-humanoids
		vector3 lookAngles;
		...
		if ( ent->client->ps.heldByClient && ent->client->ps.heldByClient <= MAX_CLIENTS ) { //then put our arm in this client's hand
			...
			else { //they left the game, perhaps?
			...
			if ( lHandBolt ) {
				...
				boltOrg.x = boltMatrix.matrix[0][3];
				boltOrg.y = boltMatrix.matrix[1][3];
				boltOrg.z = boltMatrix.matrix[2][3];
		...
		else if ( ent->client->ikStatus ) { //make sure we aren't IKing if we don't have anyone to hold onto us.
			int lHandBolt = 0;

			if ( ent && ent->inuse && ent->client && ent->ghoul2 )
				lHandBolt = trap->G2API_AddBolt( ent->ghoul2, 0, "*l_hand" );
			else { //This shouldn't happen, but just in case it does, we'll have a failsafe.
				ent->client->ikStatus = qfalse;
			}

			if ( lHandBolt )
				BG_IK_MoveArm( ent->ghoul2, lHandBolt, level.time, &ent->s, ent->client->ps.torsoAnim/*BOTH_DEAD1*/, &vec3_origin, &ent->client->ikStatus, &ent->client->ps.origin, &ent->client->ps.viewangles, &ent->modelScale, 500, qtrue );
		}
	}
	...
	else if ( ent->NPC ) { //an NPC not using a humanoid skeleton, do special angle stuff.
		if ( ent->s.eType == ET_NPC &&
			ent->s.NPC_class == CLASS_VEHICLE &&
			ent->m_pVehicle &&
			ent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER ) { //fighters actually want to take pitch and roll into account for the axial angles
			VectorCopy( &ent->client->ps.viewangles, legsAngles );
			AnglesToAxis( legsAngles, legs );
		}
		else {
...
int G_SaberLockAnim( int attackerSaberStyle, int defenderSaberStyle, int topOrSide, int lockOrBreakOrSuperBreak, int winOrLose ) {
	int baseAnim = -1;
	if ( lockOrBreakOrSuperBreak == SABERLOCK_LOCK ) {//special case: if we're using the same style and locking
		if ( attackerSaberStyle == defenderSaberStyle
			|| (attackerSaberStyle >= SS_FAST&&attackerSaberStyle <= SS_TAVION&&defenderSaberStyle >= SS_FAST&&defenderSaberStyle <= SS_TAVION) ) {//using same style
			if ( winOrLose == SABERLOCK_LOSE ) {//you want the defender's stance...
...
static qboolean WP_SabersCheckLock2( gentity_t *attacker, gentity_t *defender, sabersLockMode_t lockMode ) {
	...
	vector3	attAngles, defAngles, defDir;
	vector3	newOrg;
	vector3	attDir;
	float	diff = 0;
	trace_t trace;
	...
	if ( attacker->client->ps.fd.saberAnimLevel >= SS_FAST
		...
		&& defender->client->ps.fd.saberAnimLevel <= SS_TAVION ) {//2 single sabers?  Just do it the old way...
		switch ( lockMode ) {
		case LOCK_TOP:
		...
		idealDist = LOCK_IDEAL_DIST_JKA;//all of the new saberlocks are 46.08f from each other because Richard Lico is da MAN
		if ( lockMode == LOCK_TOP ) {//top lock
...
qboolean WP_SabersCheckLock( gentity_t *ent1, gentity_t *ent2 ) {
	...
	if ( ent1->s.eType == ET_NPC ||
		ent2->s.eType == ET_NPC ) { //if either ents is NPC, then never let an NPC lock with someone on the same playerTeam
	...
	if ( !ent1->client->ps.saberEntityNum ||
		...
		ent2->client->ps.saberInFlight ) { //can't get in lock if one of them has had the saber knocked out of his hand
		return qfalse;
	}

	if ( ent1->s.eType != ET_NPC && ent2->s.eType != ET_NPC ) { //can always get into locks with NPCs
		if ( !ent1->client->ps.duelInProgress ||
			!ent2->client->ps.duelInProgress ||
			ent1->client->ps.duelIndex != ent2->s.number ||
			ent2->client->ps.duelIndex != ent1->s.number ) { //only allow saber locking if two players are dueling with each other directly
	...
	if ( dist < 64 || dist > 6400 ) {//between 8 and 80 from each other
		return qfalse;
	}
	...
	if ( ent1->client->ps.torsoAnim == BOTH_A1_T__B_ ||
		...
		ent1->client->ps.torsoAnim == BOTH_A7_T__B_ ) {//ent1 is attacking top-down
	...
	if ( ent2->client->ps.torsoAnim == BOTH_A1_T__B_ ||
		...
		ent2->client->ps.torsoAnim == BOTH_A7_T__B_ ) {//ent2 is attacking top-down
	...
	if ( ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
		...
		ent1->client->ps.torsoAnim == BOTH_A7_TR_BL ) {//ent1 is attacking diagonally
		if ( ent2BlockingPlayer ) {//player will block this anyway
		...
		if ( ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_TL ) {//ent2 is attacking in the opposite diagonal
			return WP_SabersCheckLock2( ent1, ent2, LOCK_DIAG_TR );
		}
		if ( ent2->client->ps.torsoAnim == BOTH_A1_BR_TL ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_BL ) {//ent2 is attacking in the opposite diagonal
	...
	if ( ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
		...
		ent2->client->ps.torsoAnim == BOTH_A5_TR_BL ||
		ent2->client->ps.torsoAnim == BOTH_A6_TR_BL ||
		ent2->client->ps.torsoAnim == BOTH_A7_TR_BL ) {//ent2 is attacking diagonally
		if ( ent1BlockingPlayer ) {//player will block this anyway
		...
		if ( ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_TL ) {//ent1 is attacking in the opposite diagonal
			return WP_SabersCheckLock2( ent2, ent1, LOCK_DIAG_TR );
		}
		if ( ent1->client->ps.torsoAnim == BOTH_A1_BR_TL ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_BL ) {//ent1 is attacking in the opposite diagonal
	...
	if ( ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
		...
		ent1->client->ps.torsoAnim == BOTH_A7_TL_BR ) {//ent1 is attacking diagonally
		if ( ent2BlockingPlayer ) {//player will block this anyway
		...
		if ( ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_TR ) {//ent2 is attacking in the opposite diagonal
			return WP_SabersCheckLock2( ent1, ent2, LOCK_DIAG_TL );
		}
		if ( ent2->client->ps.torsoAnim == BOTH_A1_BL_TR ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_BR ) {//ent2 is attacking in the opposite diagonal
	...
	if ( ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
		...
		ent2->client->ps.torsoAnim == BOTH_A5_TL_BR ||
		ent2->client->ps.torsoAnim == BOTH_A6_TL_BR ||
		ent2->client->ps.torsoAnim == BOTH_A7_TL_BR ) {//ent2 is attacking diagonally
		if ( ent1BlockingPlayer ) {//player will block this anyway
		...
		if ( ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_TR ) {//ent1 is attacking in the opposite diagonal
			return WP_SabersCheckLock2( ent2, ent1, LOCK_DIAG_TL );
		}
		if ( ent1->client->ps.torsoAnim == BOTH_A1_BL_TR ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_BR ) {//ent1 is attacking in the opposite diagonal
	...
	if ( ent1->client->ps.torsoAnim == BOTH_A1__L__R ||
		...
		ent1->client->ps.torsoAnim == BOTH_A7__L__R ) {//ent1 is attacking l to r
		if ( ent2BlockingPlayer ) {//player will block this anyway
		...
		if ( ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_BL ) {//ent2 is attacking or blocking on the r
			return WP_SabersCheckLock2( ent1, ent2, LOCK_L );
		}
		return qfalse;
	}
	if ( ent2->client->ps.torsoAnim == BOTH_A1__L__R ||
		...
		ent2->client->ps.torsoAnim == BOTH_A7__L__R ) {//ent2 is attacking l to r
		if ( ent1BlockingPlayer ) {//player will block this anyway
		...
		if ( ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_BL ) {//ent1 is attacking or blocking on the r
			return WP_SabersCheckLock2( ent2, ent1, LOCK_L );
		}
		return qfalse;
	}
	...
	if ( ent1->client->ps.torsoAnim == BOTH_A1__R__L ||
		...
		ent1->client->ps.torsoAnim == BOTH_A7__R__L ) {//ent1 is attacking r to l
		if ( ent2BlockingPlayer ) {//player will block this anyway
		...
		if ( ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
			...
			ent2->client->ps.torsoAnim == BOTH_P1_S1_BR ) {//ent2 is attacking or blocking on the l
			return WP_SabersCheckLock2( ent1, ent2, LOCK_R );
		}
		return qfalse;
	}
	if ( ent2->client->ps.torsoAnim == BOTH_A1__R__L ||
		...
		ent2->client->ps.torsoAnim == BOTH_A7__R__L ) {//ent2 is attacking r to l
		if ( ent1BlockingPlayer ) {//player will block this anyway
		...
		if ( ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
			...
			ent1->client->ps.torsoAnim == BOTH_P1_S1_BR ) {//ent1 is attacking or blocking on the l
			return WP_SabersCheckLock2( ent2, ent1, LOCK_R );
		}
		return qfalse;
	}
...
static qboolean WP_GetSaberDeflectionAngle( gentity_t *attacker, gentity_t *defender, float saberHitFraction ) {
	...
	if ( (level.time - attacker->client->lastSaberStorageTime) > 500 ) { //last update was too long ago, something is happening to this client to prevent his saber from updating
		return qfalse;
	}
	if ( (level.time - defender->client->lastSaberStorageTime) > 500 ) { //ditto
		return qfalse;
	}
	...
	if ( animBasedDeflection ) {
		...
		if ( quadDiff > 4 ) {//wrap around so diff is never greater than 180 (4 * 45)
			...
			&& (defSaberLevel == attSaberLevel || Q_irand( 0, defSaberLevel - attSaberLevel ) >= 0) )//and the defender's style is stronger
		{
		...
		else {//attack hit at an angle, figure out what angle it should bounce off att
			int newQuad;
			...
			if ( newQuad == attQuadStart ) {//never come off at the same angle that we would have if the attack was not interrupted
				if ( Q_irand( 0, 1 ) ) {
					newQuad--;
				}
				else {
			...
			if ( newQuad == defQuad ) {//bounce straight back
#ifdef _DEBUG
				int attMove = attacker->client->ps.saberMove;
#endif
				attacker->client->ps.saberMove = PM_SaberBounceForAttack( attacker->client->ps.saberMove );
	...
	else { //old math-based method (probably broken)
		vector3	att_HitDir, def_BladeDir, temp;
		float	hitDot;
		...
		if ( hitDot < 0.25f && hitDot > -0.25f ) {//hit pretty much perpendicular, pop straight back
			attacker->client->ps.saberMove = PM_SaberBounceForAttack( attacker->client->ps.saberMove );
			attacker->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
			return qfalse;
		}
		else {//a deflection
			vector3	att_Right, att_Up, att_DeflectionDir;
			...
			if ( swingRDot > 0.25f ) {//deflect to right
				if ( swingUDot > 0.25f ) {//deflect to top
					attacker->client->ps.saberMove = LS_D1_TR;
				}
				else if ( swingUDot < -0.25f ) {//deflect to bottom
					attacker->client->ps.saberMove = LS_D1_BR;
				}
				else {//deflect horizontally
					attacker->client->ps.saberMove = LS_D1__R;
				}
			}
			else if ( swingRDot < -0.25f ) {//deflect to left
				if ( swingUDot > 0.25f ) {//deflect to top
				...
				else {//deflect horizontally?  Well, no such thing as straight back in my face, so use top
					if ( swingRDot > 0 ) {
						attacker->client->ps.saberMove = LS_D1_TR;
					}
					else if ( swingRDot < 0 ) {
...
static qboolean G_G2TraceCollide( trace_t *tr, vector3 *lastValidStart, vector3 *lastValidEnd, vector3 *traceMins, vector3 *traceMaxs ) { //Hit the ent with the normal trace, try the collision trace.
	G2Trace_t		G2Trace;
	gentity_t		*g2Hit;
	vector3			angles;
	int				tN = 0;
	...
	if ( !g_entities[tr->entityNum].inuse /*||
		(g_entities[tr->entityNum].s.eFlags & EF_DEAD)*/ ) { //don't do perpoly on corpses.
		return qfalse;
	}
	...
	if ( traceMins->x ||
		...
		traceMaxs->y ||
		traceMaxs->z ) {
		fRadius = (traceMaxs->x - traceMins->x) / 2.0f;
	}
	...
	if ( g2Hit && g2Hit->inuse && g2Hit->ghoul2 ) {
		...
		else { //The ghoul2 trace result matches, so copy the collision position into the trace endpos and send it back.
...
static void G_BuildSaberFaces( vector3 *base, vector3 *tip, float radius, vector3 *fwd,
	...
	while ( i < 8 ) {
		...
		else if ( i < 4 ) { //"right" surface
			d1 = fwd;
			d2 = right;
		}
		else if ( i < 6 ) { //"front" surface
			d1 = right;
			d2 = fwd;
		}
		else if ( i < 8 ) { //"back" surface
			d1 = right;
			d2 = &invFwd;
		}
...
static qboolean G_SaberFaceCollisionCheck( int fNum, saberFace_t *fList, vector3 *atkStart,
	...
	while ( i < fNum ) {
		...
		if ( G_SabCol_PointRelativeToPlane( atkStart, &side, planeEq ) !=
			G_SabCol_PointRelativeToPlane( atkEnd, &side2, planeEq ) ) { //start/end points intersect with the plane
			static vector3 extruded;
			static vector3 minPoint, maxPoint;
			static vector3 planeNormal;
			static int facing;

			VectorCopy( (vector3*)&planeEq[0], &planeNormal );
			side2 = planeNormal.x*dir.x + planeNormal.y*dir.y + planeNormal.z*dir.z;
			...
			if ( facing < 0 ) { //not intersecting.. let's try with the mins/maxs and see if they interesect on the edge plane
			...
			if ( facing >= 0 ) { //first edge is facing...
				...
				if ( facing >= 0 ) { //second edge is facing...
					...
					if ( facing >= 0 ) { //third edge is facing.. success
...
static qboolean G_SaberCollide( gentity_t *atk, gentity_t *def, vector3 *atkStart, vector3 *atkEnd, vector3 *atkMins,
	...
	if ( !g_saberBladeFaces.integer ) { //detailed check not enabled
		return qtrue;
	}

	if ( !atk->inuse || !atk->client || !def->inuse || !def->client ) { //must have 2 clients and a valid saber entity
		return qfalse;
	}
	...
	while ( i < MAX_SABERS ) {
		...
		if ( def->client->saber[i].model[0] ) { //valid saber on the defender
			...
			while ( j < def->client->saber[i].numBlades ) {
				...
				if ( (level.time - blade->storageTime) < 200 ) { //recently updated
					...
					if ( fNum > 0 ) {
						...
						if ( G_SaberFaceCollisionCheck( fNum, fList, atkStart, atkEnd, atkMins, atkMaxs, impactPoint ) ) { //collided
							return qtrue;
						}
					}
				}
...
float WP_SaberBladeLength( saberInfo_t *saber ) {//return largest length
	int	i;
	float len = 0.0f;
...
float WP_SaberLength( gentity_t *ent ) {//return largest length
	if ( !ent || !ent->client ) {
		return 0.0f;
	}
	else {
...
qboolean WP_SabersIntersect( gentity_t *ent1, int ent1SaberNum, int ent1BladeNum, gentity_t *ent2, qboolean checkDir ) {
	...
	vector3	saberBase2, saberTip2, saberBaseNext2, saberTipNext2;
	...
	for ( ent2SaberNum = 0; ent2SaberNum < MAX_SABERS; ent2SaberNum++ ) {
		if ( ent2->client->saber[ent2SaberNum].type != SABER_NONE ) {
			for ( ent2BladeNum = 0; ent2BladeNum < ent2->client->saber[ent2SaberNum].numBlades; ent2BladeNum++ ) {
				if ( ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax > 0 ) {//valid saber and this blade is on
					...
					*/
					if ( checkDir ) {//check the direction of the two swings to make sure the sabers are swinging towards each other
						vector3 saberDir1, saberDir2;
						float dot = 0.0f;
						...
						if ( DotProduct( &saberDir1, &saberDir2 ) > 0.6f ) {//sabers moving in same dir, probably didn't actually hit
							continue;
						}
						...
						if ( dot > 0.9f || dot < -0.9f ) {//too parallel to really block effectively?
							continue;
						}
					}
...
static int G_PowerLevelForSaberAnim( gentity_t *ent, int saberNum, qboolean mySaberHit ) {
	if ( !ent || !ent->client || saberNum >= MAX_SABERS ) {
		...
		if ( anim >= BOTH_A6_T__B_ && anim <= BOTH_D6_B____ ) {//dual
			return FORCE_LEVEL_2;
		}
		if ( anim >= BOTH_A7_T__B_ && anim <= BOTH_D7_B____ ) {//staff
			return FORCE_LEVEL_2;
		}
		if ( anim >= BOTH_P1_S1_T_ && anim <= BOTH_H1_S1_BR ) {//parries, knockaways and broken parries
		...
		case BOTH_A2_STABBACK1:
			...
			if ( animTimer < 450 ) {//end of anim
				return FORCE_LEVEL_0;
			}
			else if ( animTimeElapsed < 400 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		...
		case BOTH_LUNGE2_B__T_:
			...
			else if ( animTimeElapsed < 150 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		case BOTH_FORCELEAP2_T__B_:
			...
			else if ( animTimeElapsed < 550 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		case BOTH_VS_ATR_S:
		...
		case BOTH_JUMPFLIPSLASHDOWN1:
			...
			else if ( animTimeElapsed < 600 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		case BOTH_JUMPFLIPSTABDOWN:
			...
			else if ( animTimeElapsed <= 300 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		case BOTH_JUMPATTACK6:
			...
			*/
			if ( (animTimer >= 1450
				&& animTimeElapsed >= 400)
				|| (animTimer >= 400
				&& animTimeElapsed >= 1100) ) {//pretty much sideways
				return FORCE_LEVEL_3;
			}
			return FORCE_LEVEL_0;
		case BOTH_JUMPATTACK7:
			...
			else if ( animTimeElapsed < 200 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		...
		case BOTH_SPINATTACK7:
			...
			else if ( animTimeElapsed < 500 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_2;//FORCE_LEVEL_3;
		...
		case BOTH_A6_SABERPROTECT:
			...
			else if ( animTimeElapsed < 250 ) {//start of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		...
		case BOTH_PULL_IMPALE_SWING:
			...
			else if ( animTimeElapsed < 650 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_3;
		...
		case BOTH_LK_DL_S_S_SB_1_W:
			...
			else if ( animTimeElapsed < 450 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_5;
		...
		case BOTH_LK_ST_S_S_SB_1_W:
			...
			else if ( animTimeElapsed < 350 ) {//beginning of anim
				return FORCE_LEVEL_0;
			}
			return FORCE_LEVEL_5;
	...
	qboolean doDismemberment, uint32_t knockBackFlags )
{
	...
	if ( trDmg ) {//did some damage to something
		int curVictim = 0;
		int i;

		for ( i = 0; i < numVictims; i++ ) {
			if ( victimEntityNum[i] == trVictimEntityNum ) {//already hit this guy before
				curVictim = i;
				break;
			}
		}
		if ( i == numVictims ) {//haven't hit his guy before
			if ( numVictims + 1 >= MAX_SABER_VICTIMS ) {//can't add another victim at this time
				return;
			}
...
void WP_SaberApplyDamage( gentity_t *self ) {
	...
	for ( i = 0; i < numVictims; i++ ) {
		...
		if ( !dismemberDmg[i] ) {//don't do dismemberment!
	...
	for ( i = 0; i < numVictims; i++ ) {
		...
		if ( victim->client ) {
			...
			if ( npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE || npc_class == CLASS_MOUSE || npc_class == CLASS_REMOTE ||
				...
				npc_class == CLASS_INTERROGATOR || npc_class == CLASS_ATST || npc_class == CLASS_SENTRY ) { //don't make "blood" sparks for droids.
				isDroid = qtrue;
			}
		}
		...
		if ( te ) {
			...
			if ( !isDroid && (victim->client || victim->s.eType == ET_NPC ||
				...
				if ( !WP_SaberBladeUseSecondBladeStyle( &self->client->saber[saberNum], bladeNum )
					&& (self->client->saber[saberNum].saberFlags2&SFL2_NO_CLASH_FLARE) ) {//don't do clash flare
				}
				else if ( WP_SaberBladeUseSecondBladeStyle( &self->client->saber[saberNum], bladeNum )
					&& (self->client->saber[saberNum].saberFlags2&SFL2_NO_CLASH_FLARE2) ) {//don't do clash flare
				}
				else {
					if ( totalDmg[i] > SABER_NONATTACK_DAMAGE ) { //I suppose I could tie this into the saberblock event, but I'm tired of adding flags to that thing.
...
void WP_SaberRadiusDamage( gentity_t *ent, vector3 *point, float radius, int damage, float knockBack ) {
	if ( !ent || !ent->client ) {
		...
		for ( i = 0; i < numEnts; i++ ) {
			...
			if ( radiusEnt == ent ) {//Skip myself
				continue;
			}

			if ( radiusEnt->client == NULL ) {//must be a client
				if ( G_EntIsBreakable( radiusEnt->s.number ) ) {//damage breakables within range, but not as much
					G_Damage( radiusEnt, ent, ent, &vec3_origin, &radiusEnt->r.currentOrigin, 10, 0, MOD_MELEE );
				}
				continue;
			}

			if ( (radiusEnt->client->ps.eFlags2&EF2_HELD_BY_MONSTER) ) {//can't be one being held
				continue;
			}
			...
			if ( dist <= radius ) {//in range
				...
				if ( knockBack > 0 ) {//do knockback
						...
						&& !(radiusEnt->flags&FL_NO_KNOCKBACK) )//don't throw them back
					{
						...
						if ( radiusEnt->health > 0 ) {//still alive
							if ( knockbackStr > 50 ) {//close enough and knockback high enough to possibly knock down
								if ( dist < (radius*0.5f)
									|| radiusEnt->client->ps.groundEntityNum != ENTITYNUM_NONE ) {//within range of my fist or within ground-shaking range and not in the air
...
static vector3 saberClashPos = { 0 };
static vector3 saberClashNorm = { 0 };
...
void WP_SaberBounceSound( gentity_t *ent, int saberNum, int bladeNum ) {
	...
	if ( !WP_SaberBladeUseSecondBladeStyle( &ent->client->saber[saberNum], bladeNum ) && ent->client->saber[saberNum].bounceSound[0] )
		G_Sound( ent, CHAN_AUTO, ent->client->saber[saberNum].bounceSound[Q_irand( 0, 2 )] );

	else if ( WP_SaberBladeUseSecondBladeStyle( &ent->client->saber[saberNum], bladeNum ) && ent->client->saber[saberNum].bounce2Sound[0] )
		G_Sound( ent, CHAN_AUTO, ent->client->saber[saberNum].bounce2Sound[Q_irand( 0, 2 )] );

	else if ( !WP_SaberBladeUseSecondBladeStyle( &ent->client->saber[saberNum], bladeNum ) && ent->client->saber[saberNum].blockSound[0] )
	...
	qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	while ( !saberTraceDone ) {
		if ( doInterpolate && !d_saberSPStyleDamage.integer && !(japp_saberTweaks.integer & SABERTWEAK_INTERPOLATE) ) {
			...
			vector3 saberEndExtrapolated;
			if ( extrapolate ) {//extrapolate 16
				vector3 diff;
	...
	{ //this animation is that of the last attack movement, and so it should do full damage
		...
		if ( d_saberSPStyleDamage.integer ) {//SP
			float fDmg = 0.0f;
			if ( self->client->ps.saberInFlight ) {
				if ( !g_entities[self->client->ps.saberEntityNum].s.saberInFlight ) {//does less damage on the way back
					fDmg = 1.0f;
		...
		&& tr.entityNum != self->s.number && g_entities[tr.entityNum].inuse )
	{
		...
		if ( BG_StabDownAnim( self->client->ps.torsoAnim )
			...
			&& !BG_InKnockDownOnGround( &g_entities[tr.entityNum].client->ps ) ) {//stabdowns only damage people who are actually on the ground...
			return qfalse;
		}
		...
		g_entities[tr.entityNum].inuse ) { //saber clash
		...
		if ( otherOwner
			...
			if ( d_saberSPStyleDamage.integer || japp_saberTweaks.integer & SABERTWEAK_REDUCEBLOCKS ) {//use SP-style blade-collision test
				...
				if ( !G_SaberCollide( self, otherOwner, &lastValidStart,
					&lastValidEnd, &saberTrMins, &saberTrMaxs, &tr.endpos ) ) { //detailed collision did not produce results...
					return qfalse;
				}
			}
		}
		...
		if ( (self->s.eType == ET_NPC || otherOwner->s.eType == ET_NPC) && //just make sure one of us is an npc
			...
			level.gametype != GT_SIEGE ) { //don't hit your teammate's sabers if you are an NPC. It can be rather annoying.
			return qfalse;
		}
		...
		if ( saberCheckKnockdown_Smashed( &g_entities[tr.entityNum], otherOwner, self, dmg ) ) { //smashed it out of the air
			return qfalse;
		}
		...
		if ( self->client->ps.saberEntityNum
			...
			&& rSaberNum == 0
			&& saberCheckKnockdown_Smashed( &g_entities[self->client->ps.saberEntityNum], self, otherOwner, dmg ) ) { //they smashed it out of the air
			return qfalse;
		}
...
blockStuff:
		...
		{//knockaways can make fast-attacker go into a broken parry anim if the ent is using fast or med. In MP, we also randomly decide this for level 3 attacks.
			...
			(otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] < selfSaberLevel || (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == selfSaberLevel && (Q_irand( 1, 10 ) >= otherSaberLevel*1.5f || unblockable))) &&
			...
			if ( !PM_SaberInBounce( self->client->ps.saberMove ) &&
				...
				if ( otherOwner->client->ps.torsoAnim == BOTH_A1_SPECIAL
					...
					|| otherOwner->client->ps.torsoAnim == BOTH_A3_SPECIAL ) {//parry/block/break-parry bonus for single-style kata moves
					defendStr++;
				}
				...
				if ( attackAdv > 1 ) {//I won, he should knockaway
					otherOwner->client->ps.saberMove = BG_BrokenParryForAttack( otherOwner->client->ps.saberMove );
					otherOwner->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
				}
				else if ( attackAdv > 0 ) {//I won, he should bounce, I should continue
					otherOwner->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
				}
				else if ( attackAdv < 1 ) {//I lost, I get knocked away
					self->client->ps.saberMove = BG_BrokenParryForAttack( self->client->ps.saberMove );
					self->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
				}
				else if ( attackAdv < 0 ) {//I lost, I bounce off
					self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
				}
				else {//even, both bounce off
					self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
					otherOwner->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
				}
		...
		{ //block
			if ( !PM_SaberInParry( otherOwner->client->ps.saberMove ) &&
				...
				if ( !SaberAttacking( otherOwner ) ) {
					...
					(selfSaberLevel == otherSaberLevel && Q_irand( 1, 10 ) <= 2) ) { //they are attacking, and we managed to make them break
					...
					if ( selfSaberLevel == otherSaberLevel ) { //equal level, try to bounce off each other's sabers
						if ( !didOffense &&
							!PM_SaberInParry( self->client->ps.saberMove ) &&
							!PM_SaberInBrokenParry( self->client->ps.saberMove ) &&
...
void G_SPSaberDamageTraceLerped( gentity_t *self, int saberNum, int bladeNum, vector3 *baseNew, vector3 *endNew, int clipmask ) {
	vector3 baseOld, endOld;
	vector3 mp1, mp2;
	vector3 md1, md2;
	...
	if ( VectorCompare2( &baseOld, baseNew ) && VectorCompare2( &endOld, endNew ) ) {//no diff
		...
		vector3	ma1, ma2, md2ang, curBase1, curBase2;
		int	xx;
		vector3 curMD1, curMD2;//, mdDiff, dirDiff;
		float dirInc, curDirFrac;
		vector3 baseDiff, bladePointOld, bladePointNew;
...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd ) {
	...
	vector3		forward, dir, missile_dir, fwdangles = { 0 };
	trace_t		trace;
	vector3		traceTo, entDir;
	float		dot1, dot2;
	float		lookTDist = -1;
	...
	if ( !(self->client->ps.eFlags2&EF2_HELD_BY_MONSTER) ) {//lookTarget is set by and to the monster that's holding you, no other operations can change that
	...
	else if ( self->client->ps.fd.forcePowersActive&(1 << FP_LIGHTNING) ) {//can't block while zapping
		doFullRoutine = qfalse;
	}
	else if ( self->client->ps.fd.forcePowersActive&(1 << FP_DRAIN) ) {//can't block while draining
		doFullRoutine = qfalse;
	}
	else if ( self->client->ps.fd.forcePowersActive&(1 << FP_PUSH) ) {//can't block while shoving
		doFullRoutine = qfalse;
	}
	else if ( self->client->ps.fd.forcePowersActive&(1 << FP_GRIP) ) {//can't block while gripping (FIXME: or should it break the grip?  Pain should break the grip, I think...)
		doFullRoutine = qfalse;
	}

	if ( self->client->ps.weaponTime > 0 ) { //don't autoblock while busy with stuff
		return;
	}

	if ( (self->client->saber[0].saberFlags&SFL_NOT_ACTIVE_BLOCKING) ) {//can't actively block with this saber type
		return;
	}

	if ( self->health <= 0 ) {//dead don't try to block (NOTE: actual deflection happens in missile code)
		return;
	}
	if ( PM_InKnockDown( &self->client->ps ) ) {//can't block when knocked down
		return;
	}

	if ( BG_SabersOff( &self->client->ps ) && self->client->NPC_class != CLASS_BOBAFETT ) {
		if ( self->s.eType != ET_NPC ) {//player doesn't auto-activate
			doFullRoutine = qfalse;
		}
	}

	if ( self->s.eType == ET_PLAYER ) {//don't do this if already attacking!
	...
	if ( self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] > level.time ) {//can't block while gripping (FIXME: or should it break the grip?  Pain should break the grip, I think...)
		doFullRoutine = qfalse;
	}
	...
	for ( e = 0; e < numListedEntities; e++ ) {
		...
		if ( self->s.eType == ET_PLAYER &&
			...
			ent->health > 0 ) { //seems like a valid enemy to look at.
			vector3 vecSub;
			float vecLen;
			...
			if ( lookTDist == -1 || vecLen < lookTDist ) {
				...
				myEyes.z += self->client->ps.viewheight;

				trap->Trace( &tr, &myEyes, NULL, NULL, &ent->client->ps.origin, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0 );

				if ( tr.fraction == 1.0f || tr.entityNum == ent->s.number ) { //we have a clear line of sight to him, so it's all good.
					lookT = ent;
					lookTDist = vecLen;
				}
			}
		...
		if ( !doFullRoutine ) { //don't care about the rest then
			continue;
		}
		...
		if ( !(ent->inuse) )
			continue;
		if ( ent->s.eType != ET_MISSILE && !(ent->s.eFlags&EF_MISSILE_STICK) ) {//not a normal projectile
			gentity_t *pOwner;

			if ( ent->r.ownerNum < 0 || ent->r.ownerNum >= ENTITYNUM_WORLD ) { //not going to be a client then.
				continue;
			}
			...
			if ( !pOwner->client->ps.saberEntityNum ||
				...
				pOwner->client->ps.saberEntityNum != ent->s.number ) { //the saber is knocked away and/or not flying actively, or this ent is not the cl's saber ent at all
				continue;
			}
			...
			if ( ent->s.pos.trType == TR_STATIONARY && self->s.eType == ET_PLAYER ) {//nothing you can do with a stationary missile if you're the player
				continue;
			}
		}
		...
		if ( ent->s.weapon == WP_THERMAL ) {//thermal detonator!
			if ( self->NPC && dist < ent->splashRadius ) {
				if ( dist < ent->splashRadius &&
					...
					!WP_ForcePowerUsable( self, FP_PUSH )) ) {//TD is close enough to hurt me, I'm on the ground and the thing is at rest or behind me and about to blow up, or I don't have force-push so force-jump!
					//FIXME: sometimes this might make me just jump into it...?
					self->client->ps.fd.forceJumpCharge = 480;
				}
				else if ( self->client->NPC_class != CLASS_BOBAFETT ) {//FIXME: check forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
					ForceThrow( self, qfalse );
				}
			}
			continue;
		}
		else if ( ent->splashDamage && ent->splashRadius ) {//exploding missile
			...
			if ( self->s.eType == ET_PLAYER ) {//players don't auto-handle these at all
				continue;
			}
			else {
				...
				{//a placed explosive like a tripmine or detpack
					if ( InFOV3( &ent->r.currentOrigin, &self->client->renderInfo.eyePoint, &self->client->ps.viewangles, 90, 90 ) ) {//in front of me
						if ( G_ClearLOS4( self, ent ) ) {//can see it
							...
							VectorMA( &ent->r.currentOrigin, ent->r.maxs.x + 4, &throwDir, &ent->r.currentOrigin );
							VectorCopy( &ent->r.currentOrigin, &ent->s.pos.trBase );
							VectorScale( &throwDir, 300, &ent->s.pos.trDelta );
							ent->s.pos.trDelta.z += 150;
					...
					!WP_ForcePowerUsable( self, FP_PUSH )) ) {//NPCs try to evade it
					self->client->ps.fd.forceJumpCharge = 480;
				}
				else if ( self->client->NPC_class != CLASS_BOBAFETT ) {//else, try to force-throw it away
		...
		if ( ent->s.weapon != WP_SABER ) {//only block shots coming from behind
			if ( (dot1 = DotProduct( &dir, &forward )) < SABER_REFLECT_MISSILE_CONE )
				continue;
		}
		else if ( self->s.eType == ET_PLAYER ) {//player never auto-blocks thrown sabers
			continue;
		...
		if ( dist < closestDist ) {
			...
			traceTo.z = self->r.absmax.z - 4;
			trap->Trace( &trace, &ent->r.currentOrigin, &ent->r.mins, &ent->r.maxs, &traceTo, ent->s.number, ent->clipmask, qfalse, 0, 0 );
			if ( trace.allsolid || trace.startsolid || (trace.fraction < 1.0f && trace.entityNum != self->s.number && trace.entityNum != self->client->ps.saberEntityNum) ) {//okay, try one more check
				VectorNormalize2( &ent->s.pos.trDelta, &entDir );
				VectorMA( &ent->r.currentOrigin, radius, &entDir, &traceTo );
				trap->Trace( &trace, &ent->r.currentOrigin, &ent->r.mins, &ent->r.maxs, &traceTo, ent->s.number, ent->clipmask, qfalse, 0, 0 );
				if ( trace.allsolid || trace.startsolid || (trace.fraction < 1.0f && trace.entityNum != self->s.number && trace.entityNum != self->client->ps.saberEntityNum) ) {//can't hit me, ignore it
					continue;
				}
			}
			if ( self->s.eType == ET_NPC ) {//An NPC
	...
	if ( self->s.eType == ET_NPC && self->localAnimIndex <= 1 ) { //humanoid NPCs don't set angles based on server angles for looking, unlike other NPCs
		if ( self->client && self->client->renderInfo.lookTarget < ENTITYNUM_WORLD ) {
			lookT = &g_entities[self->client->renderInfo.lookTarget];
		}
	}

	if ( lookT ) { //we got a looktarget at some point so we'll assign it then.
		if ( !(self->client->ps.eFlags2&EF2_HELD_BY_MONSTER) ) {//lookTarget is set by and to the monster that's holding you, no other operations can change that
	...
	if ( !doFullRoutine ) { //then we're done now
		return;
	}
	...
	if ( incoming ) {
		if ( self->NPC /*&& !G_ControlledByPlayer( self )*/ ) {
			...
			if ( self->client->NPC_class == CLASS_BOBAFETT
				...
				&& incoming->methodOfDeath != MOD_ROCKET_HOMING ) {//a hovering Boba Fett, not a tracking rocket
				if ( !Q_irand( 0, 1 ) ) {//strafe
					self->NPC->standTime = 0;
					self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + Q_irand( 1000, 2000 );
				}
				if ( !Q_irand( 0, 1 ) ) {//go up/down
					TIMER_Set( self, "heightChange", Q_irand( 1000, 3000 ) );
					self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + Q_irand( 1000, 2000 );
				}
			}
			else if ( Jedi_SaberBlockGo( self, &self->NPC->last_ucmd, NULL, NULL, incoming, 0.0f ) != EVASION_NONE ) {//make sure to turn on your saber if it's not on
				if ( self->client->NPC_class != CLASS_BOBAFETT ) {
...
static qboolean CheckThrownSaberDamaged( gentity_t *saberent, gentity_t *saberOwner, gentity_t *ent, int dist, int returning, qboolean noDCheck ) {
	...
	if ( ent && ent->client && ent->inuse && ent->s.number != saberOwner->s.number &&
		...
		if ( veclen < dist ) { //within range
			trace_t tr;

			trap->Trace( &tr, &saberent->r.currentOrigin, NULL, NULL, &ent->client->ps.origin, saberent->s.number, MASK_SHOT, qfalse, 0, 0 );

			if ( tr.fraction == 1 || tr.entityNum == ent->s.number ) { //Slice them
				if ( !saberOwner->client->ps.isJediMaster && WP_SaberCanBlock( ent, &tr.endpos, 0, MOD_SABER, qfalse, 999 ) ) { //they blocked it
					...
					if ( saberCheckKnockdown_Thrown( saberent, saberOwner, &g_entities[tr.entityNum] ) ) { //it was knocked out of the air
						return qfalse;
					}

					if ( !returning ) { //return to owner if blocked
					...
					if ( saberOwner->client->ps.isJediMaster ) { //2x damage for the Jedi Master
		...
		if ( veclen < dist ) {
			...
			if ( ent->s.eType == ET_MOVER ) {
				...
				VectorMA( &ent->r.absmin, 0.5f, &entOrigin, &entOrigin );
			...
			if ( tr.fraction == 1 || tr.entityNum == ent->s.number ) {
				...
				if ( ent->s.eType == ET_NPC ) { //an animent
				...
				if ( ent->s.eType == ET_MOVER ) {
					if ( saberOwner
						&& saberOwner->client
						&& (saberOwner->client->saber[0].saberFlags2&SFL2_NO_CLASH_FLARE) ) {//don't do clash flare - NOTE: assumes same is true for both sabers if using dual sabers!
...
static void saberCheckRadiusDamage( gentity_t *saberent, int returning ) { //we're going to cheat and damage players within the saber's radius, just for the sake of doing things more "efficiently" (and because the saber entity has no server g2 instance)
	int i = 0;
	int dist = 0;
	gentity_t *ent;
...
static void saberMoveBack( gentity_t *ent, qboolean goingBack ) {
	...
	if ( !goingBack && ent->s.pos.trType != TR_GRAVITY ) { //acts as a fallback in case touch code fails, keeps saber from going through things between predictions
		float originalLength = 0;
		int iCompensationLength = 32;
		trace_t tr;
		vector3 mins, maxs;
		vector3 calcComp, compensatedOrigin;
		...
		compensatedOrigin.x = oldOrg.x + calcComp.x*(originalLength + iCompensationLength);
		compensatedOrigin.y = oldOrg.y + calcComp.y*(originalLength + iCompensationLength);
		compensatedOrigin.z = oldOrg.z + calcComp.z*(originalLength + iCompensationLength);
		...
		if ( (tr.fraction != 1 || tr.startsolid || tr.allsolid) && tr.entityNum != ent->r.ownerNum && !(g_entities[tr.entityNum].r.contents & CONTENTS_LIGHTSABER) ) {
			...
			if ( ent->s.pos.trType == TR_GRAVITY ) { //got blocked and knocked away in the damage func
				return;
			}

			tr.startsolid = 0;
			if ( tr.entityNum == ENTITYNUM_NONE ) { //eh, this is a filthy lie. (obviously it had to hit something or it wouldn't be in here, so we'll say it hit the world)
...
void MakeDeadSaber( gentity_t *ent ) {	//spawn a "dead" saber entity here so it looks like the saber fell out of the air.
	...
	if ( level.gametype == GT_JEDIMASTER ) { //never spawn a dead saber in JM, because the only saber on the level is really a world object
	...
	if ( tr.startsolid || tr.fraction != 1 ) {//bad position, try popping our origin up a bit
		startorg.z += 20;
		trap->Trace( &tr, &startorg, &saberent->r.mins, &saberent->r.maxs, &startorg, saberent->s.number, saberent->clipmask, qfalse, 0, 0 );
	...
	saberent->s.apos.trDelta.y = Q_irand( 200, 800 );
	saberent->s.apos.trDelta.z = Q_irand( 200, 800 );
...
void DownedSaberThink( gentity_t *saberent ) {
	...
	if ( saberOwn->client->ps.saberEntityNum ) {
		if ( saberOwn->client->ps.saberEntityNum == saberent->s.number ) { //owner shouldn't have this set if we're thinking in here. Must've fallen off a cliff and instantly respawned or something.
			notDisowned = qtrue;
		}
	...
	if ( notDisowned || saberOwn->health < 1 || !saberOwn->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] ) { //He's dead, just go back to our normal saber status
...
#ifdef _DEBUG
		if ( saberOwn->client->saberStoredIndex != saberent->s.number ) { //I'm paranoid.
		...
		if ( saberOwn->health > 0 ) { //only set this if he's alive. If dead we want to reflect the lack of saber on the corpse, as he died with his saber out.
			saberOwn->client->ps.saberInFlight = qfalse;
	...
	if ( saberOwn->client->saberKnockedTime < level.time && (saberOwn->client->pers.cmd.buttons & BUTTON_ATTACK) ) { //He wants us back
		pullBack = qtrue;
	}
	else if ( (level.time - saberOwn->client->saberKnockedTime) > MAX_LEAVE_TIME ) { //Been sitting around for too long, go back no matter what he wants.
		pullBack = qtrue;
	}

	if ( pullBack ) { //Get going back to the owner.
		saberOwn->client->ps.saberEntityNum = saberOwn->client->saberStoredIndex;
...
void saberReactivate( gentity_t *saberent, gentity_t *saberOwner ) {
	...
	saberent->s.apos.trDelta.x = 0;
	saberent->s.apos.trDelta.y = 800;
	saberent->s.apos.trDelta.z = 0;
...
void saberKnockDown( gentity_t *saberent, gentity_t *saberOwner, gentity_t *other ) {
	...
	if ( saberOwner != other ) { //if someone knocked it out of the air and it wasn't turned off, go in the direction they were facing.
		if ( other->inuse && other->client ) {
			...
			saberent->s.pos.trDelta.x = otherFwd.x*deflectSpeed;
			saberent->s.pos.trDelta.y = otherFwd.y*deflectSpeed;
			saberent->s.pos.trDelta.z = otherFwd.z*deflectSpeed;
		}
	}
...
qboolean saberKnockOutOfHand( gentity_t *saberent, gentity_t *saberOwner, const vector3 *velocity ) {
	...
	if ( !saberOwner->client->ps.saberEntityNum ) { //already gone
		return qfalse;
	}

	if ( (level.time - saberOwner->client->lastSaberStorageTime) > 50 ) { //must have a reasonably updated saber base pos
		return qfalse;
	}
...
qboolean saberCheckKnockdown_DuelLoss( gentity_t *saberent, gentity_t *saberOwner, gentity_t *other ) {
	...
	if ( !other->client->olderIsValid || (level.time - other->client->lastSaberStorageTime) >= 200 ) { //see if the spots are valid
	...
	if ( validMomentum ) {
		...
		if ( !totalDistance ) { //fine, try our own
		...
		if ( validMomentum ) {
			if ( !totalDistance ) { //try the difference between the two blades
				VectorSubtract( &saberOwner->client->lastSaberBase_Always, &other->client->lastSaberBase_Always, &dif );
				totalDistance = VectorNormalize( &dif );
			}

			if ( totalDistance ) { //if we still have no difference somehow, just let it fall to the ground when the time comes.
				if ( totalDistance < 20 ) {
					totalDistance = 20;
				}
...
qboolean saberCheckKnockdown_BrokenParry( gentity_t *saberent, gentity_t *saberOwner, gentity_t *other ) {
	...
	if ( !other->client->olderIsValid || (level.time - other->client->lastSaberStorageTime) >= 200 ) { //if we don't know which way to throw the saber based on momentum between saber positions, just don't throw it
		return qfalse;
	}

	//only knock the saber out of the hand if they're in a stronger stance I suppose. Makes strong more advantageous.
	if ( otherAttack > myAttack + 1 && Q_irand( 1, 10 ) <= 7 ) { //This would be, say, strong stance against light stance.
		doKnock = qtrue;
	}
	else if ( otherAttack > myAttack && Q_irand( 1, 10 ) <= 3 ) { //Strong vs. medium, medium vs. light
		doKnock = qtrue;
	}
	...
	if ( doKnock ) {
		...
		if ( !totalDistance ) { //fine, try our own
			if ( !saberOwner->client->olderIsValid || (level.time - saberOwner->client->lastSaberStorageTime) >= 200 ) { //if we don't know which way to throw the saber based on momentum between saber positions, just don't throw it
				return qfalse;
			}
		...
		if ( !totalDistance ) { //...forget it then.
			return qfalse;
		}
...
qboolean saberCheckKnockdown_Smashed( gentity_t *saberent, gentity_t *saberOwner, gentity_t *other, int damage ) {
	...
	if ( !saberOwner->client->ps.saberInFlight ) { //can only do this if the saber is already actually in flight
		return qfalse;
	}
	...
	if ( other
		&& other->inuse
		&& other->client
		&& BG_InExtraDefenseSaberMove( other->client->ps.saberMove ) ) { //make sure the blow was strong enough
		saberKnockDown( saberent, saberOwner, other );
		return qtrue;
	}

	if ( damage > 10 ) { //make sure the blow was strong enough
		saberKnockDown( saberent, saberOwner, other );
		return qtrue;
	}
...
void saberBackToOwner( gentity_t *saberent ) {
	...
	if ( saberOwner->health < 1 || !saberOwner->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] ) { //He's dead, just go back to our normal saber status
		saberent->touch = SaberGotHit;
		saberent->think = SaberUpdateSelf;
		saberent->genericValue5 = 0;
		saberent->nextthink = level.time;
	...
	if ( saberent->speed < level.time ) {
		...
		if ( saberOwner->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_3 ) { //allow players with high saber throw rank to control the return speed of the saber
			baseSpeed = 900;
...
void saberFirstThrown( gentity_t *saberent ) {
	...
	if ( saberOwn->health < 1 || !saberOwn->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] ) { //He's dead, just go back to our normal saber status
		saberent->touch = SaberGotHit;
		saberent->think = SaberUpdateSelf;
		saberent->genericValue5 = 0;
		saberent->nextthink = level.time;
	...
	if ( (level.time - saberOwn->client->ps.saberDidThrowTime) > 500 ) {
		if ( !(saberOwn->client->buttons & BUTTON_ALT_ATTACK) ) { //If owner releases altattack 500ms or later after throwing saber, it autoreturns
			thrownSaberTouch( saberent, saberent, NULL );
			goto runMin;
		}
		else if ( (level.time - saberOwn->client->ps.saberDidThrowTime) > 6000 ) { //if it's out longer than 6 seconds, return it
			thrownSaberTouch( saberent, saberent, NULL );
			goto runMin;
		}
	}
	...
	if ( saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_2 &&
		saberent->speed < level.time ) { //if owner is rank 3 in saber throwing, the saber goes where he points
		vector3 fwd, traceFrom, traceTo, dir;
		trace_t tr;

		AngleVectors( &saberOwn->client->ps.viewangles, &fwd, 0, 0 );

		VectorCopy( &saberOwn->client->ps.origin, &traceFrom );
		traceFrom.z += saberOwn->client->ps.viewheight;

		VectorCopy( &traceFrom, &traceTo );
		traceTo.x += fwd.x * 4096;
		traceTo.y += fwd.y * 4096;
		traceTo.z += fwd.z * 4096;

		saberMoveBack( saberent, qfalse );
		VectorCopy( &saberent->r.currentOrigin, &saberent->s.pos.trBase );

		if ( saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_3 ) { //if highest saber throw rank, we can direct the saber toward players directly by looking at them
		...
		if ( saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_3 ) { //we'll treat them to a quicker update rate if their throw rank is high enough
...
void UpdateClientRenderBolts( gentity_t *self, vector3 *renderOrigin, vector3 *renderAngles ) {
	mdxaBone_t boltMatrix;
	...
	if ( !self->ghoul2 ) {
		...
		ri->headPoint.x = boltMatrix.matrix[0][3];
		ri->headPoint.y = boltMatrix.matrix[1][3];
		ri->headPoint.z = boltMatrix.matrix[2][3];

		//right hand
		trap->G2API_GetBoltMatrix( self->ghoul2, 0, ri->handRBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, &self->modelScale );
		ri->handRPoint.x = boltMatrix.matrix[0][3];
		ri->handRPoint.y = boltMatrix.matrix[1][3];
		ri->handRPoint.z = boltMatrix.matrix[2][3];

		//left hand
		trap->G2API_GetBoltMatrix( self->ghoul2, 0, ri->handLBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, &self->modelScale );
		ri->handLPoint.x = boltMatrix.matrix[0][3];
		ri->handLPoint.y = boltMatrix.matrix[1][3];
		ri->handLPoint.z = boltMatrix.matrix[2][3];

		//chest
		trap->G2API_GetBoltMatrix( self->ghoul2, 0, ri->torsoBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, &self->modelScale );
		ri->torsoPoint.x = boltMatrix.matrix[0][3];
		ri->torsoPoint.y = boltMatrix.matrix[1][3];
		ri->torsoPoint.z = boltMatrix.matrix[2][3];
		...
		ri->crotchPoint.x = boltMatrix.matrix[0][3];
		ri->crotchPoint.y = boltMatrix.matrix[1][3];
		ri->crotchPoint.z = boltMatrix.matrix[2][3];

		//right foot
		trap->G2API_GetBoltMatrix( self->ghoul2, 0, ri->footRBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, &self->modelScale );
		ri->footRPoint.x = boltMatrix.matrix[0][3];
		ri->footRPoint.y = boltMatrix.matrix[1][3];
		ri->footRPoint.z = boltMatrix.matrix[2][3];

		//left foot
		trap->G2API_GetBoltMatrix( self->ghoul2, 0, ri->footLBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, &self->modelScale );
		ri->footLPoint.x = boltMatrix.matrix[0][3];
		ri->footLPoint.y = boltMatrix.matrix[1][3];
		ri->footLPoint.z = boltMatrix.matrix[2][3];
	}

	self->client->renderInfo.boltValidityTime = level.time;
}

void UpdateClientRenderinfo( gentity_t *self, vector3 *renderOrigin, vector3 *renderAngles ) {
	renderInfo_t *ri = &self->client->renderInfo;
	if ( ri->mPCalcTime < level.time ) {
		...
		if ( self->ghoul2 &&
			self->ghoul2 != ri->lastG2 ) { //the g2 instance changed, so update all the bolts.
		...
		if ( g_debugServerSkel.integer ) {	//Alright, I was doing this, but it's just too slow to do every frame.
		...
		ri->eyePoint.z += self->client->ps.viewheight;
	}
}
...
static void G_TossTheMofo( gentity_t *ent, vector3 *tossDir, float tossStr ) {
	if ( !ent->inuse || !ent->client ) { //no good
		return;
	}

	if ( ent->s.eType == ET_NPC && ent->s.NPC_class == CLASS_VEHICLE ) { //no, silly
		return;
	}

	VectorMA( &ent->client->ps.velocity, tossStr, tossDir, &ent->client->ps.velocity );
	ent->client->ps.velocity.z = 200;
	if ( ent->health > 0 && ent->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN &&
		BG_KnockDownable( &ent->client->ps ) &&
		G_KickDownable( ent ) ) { //if they are alive, knock them down I suppose
...
static gentity_t *G_KickTrace( gentity_t *ent, vector3 *kickDir, float kickDist, vector3 *kickEnd, int kickDamage, float kickPush ) {
	vector3	traceOrg, traceEnd, kickMins, kickMaxs;
	trace_t	trace;
	gentity_t	*hitEnt = NULL;
	VectorSet( &kickMins, -2.0f, -2.0f, -2.0f );
	VectorSet( &kickMaxs, 2.0f, 2.0f, 2.0f );
	//FIXME: variable kick height?
	if ( kickEnd && !VectorCompare( kickEnd, &vec3_origin ) ) {//they passed us the end point of the trace, just use that
		//this makes the trace flat
		VectorSet( &traceOrg, ent->r.currentOrigin.x, ent->r.currentOrigin.y, kickEnd->z );
		VectorCopy( kickEnd, &traceEnd );
	}
	else {//extrude
		VectorSet( &traceOrg, ent->r.currentOrigin.x, ent->r.currentOrigin.y, ent->r.currentOrigin.z + ent->r.maxs.z*0.5f );
	...
	if ( trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid ) {
		...
		if ( hitEnt->inuse ) {//we hit an entity
			//FIXME: don't hit same ent more than once per kick
			if ( hitEnt->takedamage ) {//hurt it
				if ( hitEnt->client ) {
			...
			if ( hitEnt->client
				...
				&& G_CanBeEnemy( ent, hitEnt ) ) {//FIXME: this should not always work
				if ( hitEnt->health <= 0 ) {//we kicked a dead guy
...
static void G_KickSomeMofos( gentity_t *ent ) {
	vector3	kickDir, kickEnd, fwdAngs;
	...
	float kickDist = (ent->r.maxs.x*1.5f) + STAFF_KICK_RANGE + 8.0f;//fudge factor of 8
	...
	if ( ent->client->ps.torsoAnim == BOTH_A7_HILT ) {
		if ( elapsedTime >= 250 && remainingTime >= 250 ) {//front
			doKick = qtrue;
			if ( ri->handRBolt != -1 ) {//actually trace to a bolt
...
static void G_GrabSomeMofos( gentity_t *self ) {
	...
	vector3 flatAng;
	vector3 pos;
	vector3 grabMins, grabMaxs;
	trace_t trace;

	if ( !self->ghoul2 || ri->handRBolt == -1 ) { //no good
		return;
	}

	VectorSet( &flatAng, 0.0f, self->client->ps.viewangles.y, 0.0f );
	...
	if ( trace.fraction != 1.0f &&
		...
		{ //grabbed an active player/npc
			...
			if ( self->client->pers.cmd.forwardmove > 0 ) { //punch grab
				tortureAnim = BOTH_KYLE_PA_1;
				correspondingAnim = BOTH_PLAYER_PA_1;
			}
			else if ( self->client->pers.cmd.forwardmove < 0 ) { //knee-throw
			...
			if ( tortureAnim == -1 || correspondingAnim == -1 ) {
				if ( self->client->ps.torsoTimer < 300 && !self->client->grappleState ) { //you failed to grab anyone, play the "failed to grab" anim
			...
			if ( self->client->ps.torsoAnim == tortureAnim ) { //providing the anim set succeeded..
				self->client->ps.weaponTime = self->client->ps.torsoTimer;
			}

			G_SetAnim( grabbed, &grabbed->client->pers.cmd, SETANIM_BOTH, correspondingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0 );
			if ( grabbed->client->ps.torsoAnim == correspondingAnim ) { //providing the anim set succeeded..
				if ( grabbed->client->ps.weapon == WP_SABER ) { //turn it off
				...
				if ( grabbed->client->ps.torsoTimer < self->client->ps.torsoTimer ) { //make sure they stay in the anim at least as long as the grabber
...
void WP_SaberPositionUpdate( gentity_t *self, usercmd_t *ucmd ) { //rww - keep the saber position as updated as possible on the server so that we can try to do realistic-looking contact stuff
	...
	mdxaBone_t	boltMatrix;
	vector3 properAngles, properOrigin;
	vector3 boltAngles, boltOrigin;
	vector3 end;
	vector3 legAxis[3];
	vector3 addVel;
	vector3 rawAngles;
	...
	if ( BG_KickingAnim( self->client->ps.legsAnim ) ) {
		...
		if ( !grappler->inuse || !grappler->client || grappler->client->grappleIndex != self->s.number ||
			...
			if ( VectorLength( &grapAng ) > 64.0f ) { //too far away, break it off
				if ( (BG_InGrappleMove( self->client->ps.torsoAnim ) && self->client->ps.torsoTimer > 100) ||
				...
				if ( self->client->grappleState >= 20 ) { //grapplee
					//try to position myself at the correct distance from my grappler
					float idealDist;
					vector3 gFwd, idealSpot;
					trace_t trace;

					if ( grappler->client->ps.torsoAnim == BOTH_KYLE_PA_1 ) { //grab punch
						idealDist = 46.0f;
					}
					else { //knee-throw
						idealDist = 34.0f;
					...
					if ( !trace.startsolid && !trace.allsolid && trace.fraction == 1.0f ) { //go there
						G_SetOrigin( self, &idealSpot );
						VectorCopy( &idealSpot, &self->client->ps.origin );
					}
				}
				else if ( self->client->grappleState >= 1 ) { //grappler
					if ( grappler->client->ps.weapon == WP_SABER ) { //make sure their saber is shut off
					...
					if ( self->client->ps.torsoAnim == BOTH_KYLE_PA_1 ) { //grab punch
						if ( self->client->grappleState == 1 ) { //smack
						...
						else if ( self->client->grappleState == 2 ) { //smack!
							...
							if ( self->client->ps.torsoTimer < 1300 ) {
								...
								if ( grappler->health > 0 ) { //if still alive knock them down
					...
					else if ( self->client->ps.torsoAnim == BOTH_KYLE_PA_2 ) { //knee throw
						if ( self->client->grappleState == 1 ) { //knee to the face
						...
						else if ( self->client->grappleState == 2 ) { //smashed on the ground
	...
	if ( self->client->ps.saberThrowDelay < level.time ) {
		if ( (self->client->saber[0].saberFlags&SFL_NOT_THROWABLE) ) {//cant throw it normally!
			if ( (self->client->saber[0].saberFlags&SFL_SINGLE_BLADE_THROWABLE) ) {//but can throw it if only have 1 blade on
				if ( self->client->saber[0].numBlades > 1
					&& self->client->ps.saberHolstered == 1 ) {//have multiple blades and only one blade on
					self->client->ps.saberCanThrow = qtrue;//qfalse;
					//huh? want to be able to throw then right?
				}
				else {//multiple blades on, can't throw
					self->client->ps.saberCanThrow = qfalse;
				}
			}
			else {//never can throw it
				self->client->ps.saberCanThrow = qfalse;
			}
		}
		else {//can throw it!
...
nextStep:
	...
	if ( self->client->ps.velocity.x < 0 ) {
		fVSpeed += (-self->client->ps.velocity.x);
	}
	else {
		fVSpeed += self->client->ps.velocity.x;
	}
	if ( self->client->ps.velocity.y < 0 ) {
		fVSpeed += (-self->client->ps.velocity.y);
	}
	else {
		fVSpeed += self->client->ps.velocity.y;
	}
	if ( self->client->ps.velocity.z < 0 ) {
		fVSpeed += (-self->client->ps.velocity.z);
	}
	else {
		fVSpeed += self->client->ps.velocity.z;
	}
	...
	properOrigin.x += addVel.x*fVSpeed;
	properOrigin.y += addVel.y*fVSpeed;
	properOrigin.z += addVel.z*fVSpeed;

	properAngles.pitch = 0;
	if ( self->s.number < MAX_CLIENTS && self->client->ps.m_iVehicleNum ) {
		...
		if ( vehEnt->inuse && vehEnt->client && vehEnt->m_pVehicle ) {
			properAngles.yaw = vehEnt->m_pVehicle->m_vOrientation->yaw;
		}
		else {
			properAngles.yaw = self->client->ps.viewangles.yaw;
			vehEnt = NULL;
		}
	}
	else {
	...
	if ( !clientOverride ) { //if we get the client instance we don't need to do this
	...
	if ( returnAfterUpdate && saberNum ) { //We don't even need to do GetBoltMatrix if we're only in here to keep the g2 server instance in sync
	...
	if ( self->client->lastSaberStorageTime && (level.time - self->client->lastSaberStorageTime) < 200 ) { //alright
	...
	if ( self->client->ps.saberEntityNum ) {
		//I guess it's good to keep the position updated even when contents are 0
		if ( mySaber && ((mySaber->r.contents & CONTENTS_LIGHTSABER) || mySaber->r.contents == 0) && !self->client->ps.saberInFlight ) { //place it roughly in the middle of the saber..
	...
	if ( self->client->ps.saberInFlight ) { //do the thrown-saber stuff
		...
		if ( saberent ) {
			...
			else if ( self->client->ps.saberEntityNum ) //only do this stuff if your saber is active and has not been knocked out of the air.
			{
				...
				if ( saberent->genericValue5 == PROPER_THROWN_VALUE ) { //return to the owner now, this is a bad state to be in for here..
					saberent->genericValue5 = 0;
					saberent->think = SaberUpdateSelf;
					saberent->nextthink = level.time;
					WP_SaberRemoveG2Model( saberent );
	...
	if ( !BG_SabersOff( &self->client->ps ) ) {
		...
		if ( self->client->ps.saberLockTime > level.time && self->client->ps.saberEntityNum ) {
			...
			while ( rSaberNum < MAX_SABERS ) {
				rBladeNum = 0;
				while ( rBladeNum < self->client->saber[rSaberNum].numBlades ) { //Don't bother updating the bolt for each blade for this, it's just a very rough fallback method for during saberlocks
		...
		if ( self->client->ps.saberInFlight ) { //if saber is thrown then only do the standard stuff for the left hand saber
			if ( !self->client->ps.saberEntityNum ) { //however, if saber is not in flight but rather knocked away, our left saber is off, and thus we may do nothing.
				rSaberNum = 1;//was 2?
			}
			else {//thrown saber still in flight, so do damage
				rSaberNum = 0;//was 1?
			}
		}
			...
			*/
			...
			while ( rBladeNum < self->client->saber[rSaberNum].numBlades ) {
				...
				if ( rSaberNum == 0 && self->client->ps.saberInFlight ) {
					if ( !self->client->ps.saberEntityNum ) {//dropped it... shouldn't get here, but...
						//assert(0);
						...
						if ( saberent->s.saberInFlight ) {//spinning
				...
				if ( self->client->hasCurrentPosition && d_saberInterpolate.integer ) {
					if ( self->client->ps.weaponTime <= 0 ) { //rww - 07/17/02 - don't bother doing the extra stuff unless actually attacking. This is in attempt to save CPU.
						...
						if ( !g_saberTraceSaberFirst.integer ) {
							...
							!self->client->ps.duelInProgress ) { //if value is >= 2, and not in a duel, skip
							skipSaberTrace = qtrue;
						}

						if ( skipSaberTrace ) { //skip the saber-contents-only trace and get right to the full trace
							trMask = (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT);
						}
						else {
							while ( sN < MAX_CLIENTS ) {
								if ( g_entities[sN].inuse && g_entities[sN].client && g_entities[sN].r.linked && g_entities[sN].health > 0 && (g_entities[sN].r.contents & CONTENTS_BODY) ) { //Take this mask off before the saber trace, because we want to hit the saber first
						...
						while ( !gotHit ) {
							if ( !CheckSaberDamage( self, rSaberNum, rBladeNum, &boltOrigin, &end, qfalse, trMask, qfalse ) ) {
								if ( !CheckSaberDamage( self, rSaberNum, rBladeNum, &boltOrigin, &end, qtrue, trMask, qfalse ) ) {
									vector3 oldSaberStart, oldSaberEnd;
									vector3 saberAngleNow, saberAngleBefore;
									vector3 saberMidDir, saberMidAngle, saberMidPoint, saberMidEnd;
									vector3 saberSubBase;
									...
									deltaX = AngleDelta( saberAngleBefore.x, saberAngleNow.x );
									deltaY = AngleDelta( saberAngleBefore.y, saberAngleNow.y );
									deltaZ = AngleDelta( saberAngleBefore.z, saberAngleNow.z );

									if ( (deltaX != 0 || deltaY != 0 || deltaZ != 0) && deltaX < 180 && deltaY < 180 && deltaZ < 180 && (BG_SaberInAttack( self->client->ps.saberMove ) || PM_SaberInTransition( self->client->ps.saberMove )) ) { //don't go beyond here if we aren't attacking/transitioning or the angle is too large.
										//and don't bother if the angle is the same
										saberMidAngle.x = saberAngleBefore.x + (deltaX / 2);
										saberMidAngle.y = saberAngleBefore.y + (deltaY / 2);
										saberMidAngle.z = saberAngleBefore.z + (deltaZ / 2);
										...
										saberMidPoint.x = boltOrigin.x + (saberSubBase.x*0.5f);
										saberMidPoint.y = boltOrigin.y + (saberSubBase.y*0.5f);
										saberMidPoint.z = boltOrigin.z + (saberSubBase.z*0.5f);

										AngleVectors( &saberMidAngle, &saberMidDir, 0, 0 );
										saberMidEnd.x = saberMidPoint.x + saberMidDir.x*self->client->saber[rSaberNum].blade[rBladeNum].lengthMax;
										saberMidEnd.y = saberMidPoint.y + saberMidDir.y*self->client->saber[rSaberNum].blade[rBladeNum].lengthMax;
										saberMidEnd.z = saberMidPoint.z + saberMidDir.z*self->client->saber[rSaberNum].blade[rBladeNum].lengthMax;
							...
							if ( g_saberTraceSaberFirst.integer ) {
								sN = 0;
								while ( sN < MAX_CLIENTS ) {
									if ( clientUnlinked[sN] ) { //Make clients clip properly again.
...
finalUpdate:
	if ( clientOverride ) { //if we get the client instance we don't even need to bother setting anims and stuff
		return;
	}
...
void WP_SaberBlock( gentity_t *playerent, vector3 *hitloc, qboolean missileBlock ) {
	vector3 diff, fwdangles = { 0, 0, 0 }, right;
	float rightdot;
	float zdiff;
	...
	fwdangles.y = playerent->client->ps.viewangles.y;
	// Ultimately we might care if the shot was ahead or behind, but for now, just quadrant is fine.
	AngleVectors( &fwdangles, NULL, &right, NULL );

	rightdot = DotProduct( &right, &diff ) + RandFloat( -0.2f, 0.2f );
	zdiff = hitloc->z - playerent->client->ps.origin.z + Q_irand( -8, 8 );

	// Figure out what quadrant the block was in.
	if ( zdiff > 24 ) {	// Attack from above
		if ( Q_irand( 0, 1 ) )
			playerent->client->ps.saberBlocked = BLOCKED_TOP;
		else
			playerent->client->ps.saberBlocked = BLOCKED_UPPER_LEFT;
	}
	else if ( zdiff > 13 ) {	// The upper half has three viable blocks...
		if ( rightdot > 0.25f ) {	// In the right quadrant...
			if ( Q_irand( 0, 1 ) )
				playerent->client->ps.saberBlocked = BLOCKED_UPPER_LEFT;
			else
	...
	else {	// The lower half is a bit iffy as far as block coverage.  Pick one of the "low" ones at random.
		if ( Q_irand( 0, 1 ) )
			playerent->client->ps.saberBlocked = BLOCKED_LOWER_RIGHT;
		else
			playerent->client->ps.saberBlocked = BLOCKED_LOWER_LEFT;
...
qboolean HasSetSaberOnly( void ) {
	...
	if ( level.gametype == GT_JEDIMASTER ) { //set to 0
		return qfalse;
	}
