...
void WP_InitForcePowers( gentity_t *ent ) {
	...
	if ( ent->client->pers.adminData.empowered ) {
		...
		for ( i = 0; i < NUM_FORCE_POWERS; i++ ) {
			ent->client->pers.adminData.forcePowerBaseLevel[i] = ent->client->ps.fd.forcePowerBaseLevel[i];		ent->client->ps.fd.forcePowerBaseLevel[i] = 3;
			ent->client->pers.adminData.forcePowerLevel[i] = ent->client->ps.fd.forcePowerLevel[i];				ent->client->ps.fd.forcePowerLevel[i] = 3;
			ent->client->ps.fd.forcePowersKnown |= (1 << i);
		}
	}
}
...
int ForcePowerUsableOn( gentity_t *attacker, gentity_t *other, forcePowers_t forcePower ) {
	...
	if ( forcePower == FP_GRIP ) {
		if ( other && other->client &&
			(other->client->ps.fd.forcePowersActive & (1 << FP_ABSORB)) ) { //don't allow gripping to begin with if they are absorbing
			...
			BG_SaberInSpecial( other->client->ps.saberMove ) ) { //don't grip person while they are in a special or some really bad things can happen.
			return 0;
		}
	}
	...
	if ( other && other->client && other->s.eType == ET_NPC &&
		other->s.NPC_class == CLASS_VEHICLE ) { //can't use the force on vehicles.. except lightning
	...
	if ( other && other->client && other->s.eType == ET_NPC &&
		level.gametype == GT_SIEGE ) { //can't use powers at all on npc's normally in siege...
		return 0;
	}
...
qboolean WP_ForcePowerAvailable( gentity_t *self, forcePowers_t forcePower, int overrideAmt ) {
	...
	if ( self->client->ps.fd.forcePowersActive & (1 << forcePower) ) { //we're probably going to deactivate it..
		return qtrue;
	}
	...
	if ( (forcePower == FP_DRAIN || forcePower == FP_LIGHTNING) &&
		self->client->ps.fd.forcePower >= 25 ) { //it's ok then, drain/lightning are actually duration
		return qtrue;
	}
...
qboolean WP_ForcePowerInUse( gentity_t *self, forcePowers_t forcePower ) {
	if ( (self->client->ps.fd.forcePowersActive & (1 << forcePower)) ) {//already using this power
		return qtrue;
	}
...
qboolean WP_ForcePowerUsable( gentity_t *self, forcePowers_t forcePower ) {
	...
	if ( self->client->ps.pm_flags & PMF_FOLLOW ) { //specs can't use powers through people
		return qfalse;
	}
	...
	if ( !(self->client->ps.fd.forcePowersKnown & (1 << forcePower)) ) {//don't know this power
		return qfalse;
	}
	...
	if ( !self->client->ps.saberHolstered ) {
		...
		if ( self->client->saber[0].model[0] ) {//both sabers on
			...
			if ( (self->client->saber[1].forceRestrictions&(1 << forcePower)) ) {//this power is verboten when using this saber
				return qfalse;
			}
		}
	}
...
int WP_AbsorbConversion( gentity_t *attacked, int atdAbsLevel, gentity_t *attacker, int atPower, int atPowerLevel, int atForceSpent ) {
	...
	if ( atPower != FP_LIGHTNING &&
		...
		atPower != FP_GRIP &&
		atPower != FP_PUSH &&
		atPower != FP_PULL ) { //Only these powers can be absorbed
		return -1;
	}

	if ( !atdAbsLevel ) { //looks like attacker doesn't have any absorb power
		return -1;
	}

	if ( !(attacked->client->ps.fd.forcePowersActive & (1 << FP_ABSORB)) ) { //absorb is not active
		return -1;
	}
...
void WP_ForcePowerRegenerate( gentity_t *self, int overrideAmt ) { //called on a regular interval to regenerate force power.
	if ( !self->client ) {
		return;
	}

	if ( overrideAmt ) { //custom regen amount
		self->client->ps.fd.forcePower += overrideAmt;
	}
	else { //otherwise, just 1
		self->client->ps.fd.forcePower++;
	}

	if ( self->client->ps.fd.forcePower > self->client->ps.fd.forcePowerMax ) { //cap it off at the max (default 100)
		self->client->ps.fd.forcePower = self->client->ps.fd.forcePowerMax;
	}
}

void WP_ForcePowerStart( gentity_t *self, forcePowers_t forcePower, int overrideAmt ) { //activate the given force power
	int	duration = 0;
	...
	if ( BG_FullBodyTauntAnim( self->client->ps.legsAnim ) ) {//stop taunt
		self->client->ps.legsTimer = 0;
	}
	if ( BG_FullBodyTauntAnim( self->client->ps.torsoAnim ) ) {//stop taunt
	...
	else if ( (int)forcePower != FP_GRIP && (int)forcePower != FP_DRAIN ) { //grip and drain drain as damage is done
...
void ForceGrip( gentity_t *self ) {
	...
	tto.x = tfrom.x + fwd.x*MAX_GRIP_DISTANCE;
	tto.y = tfrom.y + fwd.y*MAX_GRIP_DISTANCE;
	tto.z = tfrom.z + fwd.z*MAX_GRIP_DISTANCE;
		...
		if ( g_entities[tr.entityNum].s.number < MAX_CLIENTS && g_entities[tr.entityNum].client->ps.m_iVehicleNum ) { //a player on a vehicle
			...
			if ( vehEnt->inuse && vehEnt->client && vehEnt->m_pVehicle ) {
				if ( vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
					vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL ) { //push the guy off
...
void ForceSpeed( gentity_t *self, int forceDuration ) {
	...
	if ( self->client->holdingObjectiveItem >= MAX_CLIENTS
		&& self->client->holdingObjectiveItem < ENTITYNUM_WORLD ) {//holding Siege item
		if ( g_entities[self->client->holdingObjectiveItem].genericValue15 ) {//disables force powers
			return;
		}
	}
...
void ForceLightning( gentity_t *self ) {
	...
	if ( self->client->ps.fd.forcePowerDebounce[FP_LIGHTNING] > level.time ) {//stops it while using it and also after using it, up to 3 second delay
		return;
	}
...
void ForceLightningDamage( gentity_t *self, gentity_t *traceEnt, vector3 *dir, vector3 *impactPoint ) {
	...
	if ( traceEnt && traceEnt->takedamage ) {
		...
		if ( traceEnt->client ) {//an enemy or object
			if ( traceEnt->client->noLightningTime >= level.time ) { //give them power and don't hurt them.
			...
			if ( ForcePowerUsableOn( self, traceEnt, FP_LIGHTNING ) ) {
				...
				if ( self->client->ps.weapon == WP_MELEE
					&& self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 ) {//2-handed lightning
				...
				if ( traceEnt->client ) {
					...
					if ( traceEnt->client->ps.electrifyTime < (level.time + 400) ) { //only update every 400ms to reduce bandwidth usage (as it is passing a 32-bit time value)
						traceEnt->client->ps.electrifyTime = level.time + 800;
					}
					if ( traceEnt->client->ps.powerups[PW_CLOAKED] ) {//disable cloak temporarily
...
void ForceShootLightning( gentity_t *self ) {
	...
	if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 ) {//arc
		vector3	center, mins, maxs, dir, ent_org, size, v;
		...
		for ( e = 0; e < numListedEntities; e++ ) {
			...
			//in PVS?
			if ( !traceEnt->r.bmodel && !trap->InPVS( &ent_org, &self->client->ps.origin ) ) {//must be in PVS
				continue;
			}

			//Now check and see if we can actually hit it
			trap->Trace( &tr, &self->client->ps.origin, &vec3_origin, &vec3_origin, &ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
			if ( tr.fraction < 1.0f && tr.entityNum != traceEnt->s.number ) {//must have clear LOS
				continue;
			}
...
void ForceDrain( gentity_t *self ) {
	...
	if ( self->client->ps.fd.forcePowerDebounce[FP_DRAIN] > level.time ) {//stops it while using it and also after using it, up to 3 second delay
		return;
	}
...
void ForceDrainDamage( gentity_t *self, gentity_t *traceEnt, vector3 *dir, vector3 *impactPoint ) {
	gentity_t *tent;
...
int ForceShootDrain( gentity_t *self ) {
	...
	if ( self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2 ) {//arc
		vector3	center, mins, maxs, dir, ent_org, size, v;
...
void ForceJumpCharge( gentity_t *self, usercmd_t *ucmd ) { //I guess this is unused now. Was used for the "charge" jump type.
	...
	if ( self->client->ps.fd.forceJumpCharge / forceJumpChargeInterval / (FORCE_JUMP_CHARGE_TIME / FRAMETIME)*forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]][FP_LEVITATION] > self->client->ps.fd.forcePower ) {//can't use more than you have
		G_MuteSound( self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50], CHAN_VOICE );
...
int WP_GetVelocityForForceJump( gentity_t *self, vector3 *jumpVel, usercmd_t *ucmd ) {
	...
	if ( self->client->ps.fd.forceJumpCharge < JUMP_VELOCITY + 40 ) { //give him at least a tiny boost from just a tap
		self->client->ps.fd.forceJumpCharge = JUMP_VELOCITY + 400;
	}

	if ( self->client->ps.velocity.z < -30 ) { //so that we can get a good boost when force jumping in a fall
		self->client->ps.velocity.z = -30;
	}

	VectorMA( &self->client->ps.velocity, pushFwd, &forward, jumpVel );
	VectorMA( &self->client->ps.velocity, pushRt, &right, jumpVel );
	jumpVel->z += self->client->ps.fd.forceJumpCharge;
...
void ForceJump( gentity_t *self, usercmd_t *ucmd ) {
	...
	//FIXME: sound effect
	self->client->ps.fd.forceJumpZStart = self->client->ps.origin.z;//remember this for when we land
	...
	//Raz: sv_fps dependent jump heights https://github.com/Razish/OpenJK/commit/808e74dd362e2b407845d7adc8e7dea984a72129
...
qboolean ForceTelepathyCheckDirectNPCTarget( gentity_t *self, trace_t *tr, qboolean *tookPower ) {
	...
	if ( targetLive && traceEnt->NPC ) {//hit an organic non-player
		...
		if ( G_ActivateBehavior( traceEnt, BSET_MINDTRICK ) ) {//activated a script on him
			...
			|| (!self->NPC && !OnSameTeam( traceEnt, self )) ) {//an enemy
			int override = 0;
		...
		if ( self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_1 && tr->fraction * 2048 > 64 ) {//don't create a diversion less than 64 from you of if at power level 1
...
void ForceTelepathy( gentity_t *self ) {
	trace_t tr;
	vector3 tto, thispush_org, a;
	vector3 mins, maxs, fwdangles, forward, right, center;
	int i;
	float visionArc = 0;
	float radius = MAX_TRICK_DISTANCE;
	...
	if ( self->client->ps.powerups[PW_REDFLAG] ||
		self->client->ps.powerups[PW_BLUEFLAG] ) { //can't mindtrick while carrying the flag
		return;
	}
	...
	if ( ForceTelepathyCheckDirectNPCTarget( self, &tr, &tookPower ) ) {//hit an NPC directly
		self->client->ps.forceAllowDeactivateTime = level.time + 1500;
	...
	else	//level 2 & 3
	{
		...
		while ( e < numListedEntities ) {
			...
			if ( ent ) { //not in the arc, don't consider it
				if ( ent->client ) {
				...
				tto.z += self->client->ps.viewheight;
				...
				else if ( !InFieldOfVision( &self->client->ps.viewangles, visionArc, &a ) ) { //only bother with arc rules if the victim is a client
					entityList[e] = ENTITYNUM_NONE;
				}
...
qboolean CanCounterThrow( gentity_t *self, gentity_t *thrower, qboolean pull ) {
	...
	if ( level.gametype == GT_SIEGE && pull && thrower && thrower->client ) {
		vector3 d;
		float a;
		...
		if ( a > 60.0f || a < -60.0f ) { //if facing more than 60 degrees away they cannot defend
			return 0;
		}
	}
	...
	if ( self->client->ps.groundEntityNum == ENTITYNUM_NONE ) { //you cannot counter a push/pull if you're in the air
		return 0;
	}
...
void ForceThrow( gentity_t *self, qboolean pull ) {
	...
	if ( pull ) {
		...
		if ( self->client->ps.forceHandExtend == HANDEXTEND_NONE ) {
			...
			if ( level.gametype == GT_SIEGE && self->client->ps.weapon == WP_SABER ) {//hold less so can attack right after a pull
	...
	if ( !powerLevel ) { //Shouldn't have made it here..
		return;
	}
	...
	if ( powerLevel == FORCE_LEVEL_1 ) { //can only push/pull targeted things at level 1
		VectorCopy( &self->client->ps.origin, &tfrom );
		tfrom.z += self->client->ps.viewheight;
		AngleVectors( &self->client->ps.viewangles, &fwd, NULL, NULL );
		tto.x = tfrom.x + fwd.x * radius / 2;
	...
	for ( e = 0; e < numListedEntities; e++ ) {
		...
		if ( ent->s.eType != ET_MISSILE ) {
							...
							if ( Q_stricmp( "lightsaber", ent->classname ) != 0 ) {//not a lightsaber
								if ( Q_stricmp( "func_door", ent->classname ) != 0 || !(ent->spawnflags & 2/*MOVER_FORCE_ACTIVATE*/) ) {//not a force-usable door
									if ( Q_stricmp( "func_static", ent->classname ) != 0 || (!(ent->spawnflags & 1/*F_PUSH*/) && !(ent->spawnflags & 2/*F_PULL*/)) ) {//not a force-usable func_static
										if ( Q_stricmp( "limb", ent->classname ) ) {//not a limb
											continue;
										}
									}
								}
								else if ( ent->moverState != MOVER_POS1 && ent->moverState != MOVER_POS2 ) {//not at rest
									continue;
								}
							}
						}
			...
			if ( ent->s.pos.trType == TR_STATIONARY && (ent->s.eFlags&EF_MISSILE_STICK) ) {//can't force-push/pull stuck missiles (detpacks, tripmines)
				continue;
			}
			if ( ent->s.pos.trType == TR_STATIONARY && ent->s.weapon != WP_THERMAL ) {//only thermal detonators can be pushed once stopped
				continue;
			}
		}
		...
		//in PVS?
		if ( !ent->r.bmodel && !trap->InPVS( &ent_org, &self->client->ps.origin ) ) {//must be in PVS
			continue;
		}

		//really should have a clear LOS to this thing...
		trap->Trace( &tr, &self->client->ps.origin, &vec3_origin, &vec3_origin, &ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
		if ( tr.fraction < 1.0f && tr.entityNum != ent->s.number ) {//must have clear LOS
			//try from eyes too before you give up
			vector3 eyePoint;
			VectorCopy( &self->client->ps.origin, &eyePoint );
			eyePoint.z += self->client->ps.viewheight;
		...
		for ( x = 0; x < ent_count; x++ ) {
			...
			if ( push_list[x]->client ) {//FIXME: make enemy jedi able to hunker down and resist this?
				...
				if ( push_list[x]->client->pers.cmd.forwardmove ||
					push_list[x]->client->pers.cmd.rightmove ) { //if you are moving, you get one less level of defense
					otherPushPower--;
				...
				if ( (modPowerLevel > otherPushPower || push_list[x]->client->ps.m_iVehicleNum) && push_list[x]->client ) {
					if ( modPowerLevel == FORCE_LEVEL_3 &&
						...
						if ( BG_KnockDownable( &push_list[x]->client->ps ) &&
							dirLen <= (64 * ((modPowerLevel - otherPushPower) - 1)) ) { //can only do a knockdown if fairly close
							push_list[x]->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
							...
							dirLen <= 128.0f ) { //a player on a vehicle
				...
				if ( push_list[x]->client ) {
					...
					if ( self->client->ps.fd.forceGripBeingGripped > level.time ) { //force the enemy to stop gripping me if I managed to push him
						if ( push_list[x]->client->ps.fd.forceGripEntityNum == self->s.number ) {
							if ( modPowerLevel >= push_list[x]->client->ps.fd.forcePowerLevel[FP_GRIP] ) { //only break the grip if our push/pull level is >= their grip level
					...
					push_list[x]->client->ps.velocity.x = pushDir.x*pushPowerMod;
					push_list[x]->client->ps.velocity.y = pushDir.y*pushPowerMod;

					if ( (int)push_list[x]->client->ps.velocity.z == 0 ) { //if not going anywhere vertically, boost them up a bit
						push_list[x]->client->ps.velocity.z = pushDir.z*pushPowerMod;

						if ( push_list[x]->client->ps.velocity.z < 128 ) {
							push_list[x]->client->ps.velocity.z = 128;
						}
					}
					else {
						push_list[x]->client->ps.velocity.z = pushDir.z*pushPowerMod;
			...
			{
				if ( pull ) {//deflect rather than reflect?
				}
				else {
			...
			else if ( !Q_stricmp( "func_static", push_list[x]->classname ) ) {//force-usable func_static
			...
			else if ( !Q_stricmp( "func_door", push_list[x]->classname ) && (push_list[x]->spawnflags & 2) ) {//push/pull the door
				vector3	pos1, pos2;
				vector3	trFrom;

				VectorCopy( &self->client->ps.origin, &trFrom );
				trFrom.z += self->client->ps.viewheight;

				AngleVectors( &self->client->ps.viewangles, &forward, NULL, NULL );
				VectorNormalize( &forward );
				VectorMA( &trFrom, radius, &forward, &end );
				trap->Trace( &tr, &trFrom, &vec3_origin, &vec3_origin, &end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
				if ( tr.entityNum != push_list[x]->s.number || tr.fraction == 1.0f || tr.allsolid || tr.startsolid ) {//must be pointing right at it
					continue;
				}

				if ( VectorCompare( &vec3_origin, &push_list[x]->s.origin ) ) {//does not have an origin brush, so pos1 & pos2 are relative to world origin, need to calc center
					VectorSubtract( &push_list[x]->r.absmax, &push_list[x]->r.absmin, &size );
					VectorMA( &push_list[x]->r.absmin, 0.5f, &size, &center );
					if ( (push_list[x]->spawnflags & 1) && push_list[x]->moverState == MOVER_POS1 ) {//if at pos1 and started open, make sure we get the center where it *started* because we're going to add back in the relative values pos1 and pos2
						VectorSubtract( &center, &push_list[x]->pos1, &center );
					}
					else if ( !(push_list[x]->spawnflags & 1) && push_list[x]->moverState == MOVER_POS2 ) {//if at pos2, make sure we get the center where it *started* because we're going to add back in the relative values pos1 and pos2
				...
				else {//actually has an origin, pos1 and pos2 are absolute
					VectorCopy( &push_list[x]->r.currentOrigin, &center );
					VectorCopy( &push_list[x]->pos1, &pos1 );
					VectorCopy( &push_list[x]->pos2, &pos2 );
				}

				if ( Distance( &pos1, &trFrom ) < Distance( &pos2, &trFrom ) ) {//pos1 is closer
					if ( push_list[x]->moverState == MOVER_POS1 ) {//at the closest pos
						if ( pull ) {//trying to pull, but already at closest point, so screw it
							continue;
						}
					}
					else if ( push_list[x]->moverState == MOVER_POS2 ) {//at farthest pos
						if ( !pull ) {//trying to push, but already at farthest point, so screw it
							continue;
						}
					}
				}
				else {//pos2 is closer
					if ( push_list[x]->moverState == MOVER_POS1 ) {//at the farthest pos
						if ( !pull ) {//trying to push, but already at farthest point, so screw it
							continue;
						}
					}
					else if ( push_list[x]->moverState == MOVER_POS2 ) {//at closest pos
						if ( pull ) {//trying to pull, but already at closest point, so screw it
							continue;
						}
					}
			...
			else if ( Q_stricmp( "func_button", push_list[x]->classname ) == 0 ) {//pretend you pushed it
				...
				if ( japp_allowFlagPull.integer && push_list[x]->s.eType == ET_ITEM && (!strcmp( push_list[x]->classname, "team_CTF_redflag" ) || !strcmp( push_list[x]->classname, "team_CTF_blueflag" )) ) {//Flags
					...
					VectorNormalize( &forward );
					VectorScale( &forward, -550.0f, &end );
					VectorCopy( &end, &push_list[x]->s.pos.trDelta );
					push_list[x]->s.pos.trType = TR_GRAVITY;
...
void WP_ForcePowerStop( gentity_t *self, forcePowers_t forcePower ) {
	...
	case FP_GRIP:
		...
		if ( self->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 &&
			...
			(level.time - g_entities[self->client->ps.fd.forceGripEntityNum].client->ps.fd.forceGripStarted) > 500 ) { //if we had our throat crushed in for more than half a second, gasp for air when we're let go
		...
		break;
	case FP_LIGHTNING:
		if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] < FORCE_LEVEL_2 ) {//don't do it again for 3 seconds, minimum... FIXME: this should be automatic once regeneration is slower (normal)
	...
	case FP_PROTECT:
		...
		break;
	case FP_DRAIN:
		if ( self->client->ps.fd.forcePowerLevel[FP_DRAIN] < FORCE_LEVEL_2 ) {//don't do it again for 3 seconds, minimum...
...
void DoGripAction( gentity_t *self, forcePowers_t forcePower ) {
	...
	trace_t tr;
	vector3 a;
	vector3 fwd, fwd_o, start_o, nvel;
	...
	if ( self->client->ps.fd.forcePowerDebounce[FP_GRIP] < level.time ) { //2 damage per second while choking, resulting in 10 damage total (not including The Squeeze<tm>)
	...
	if ( gripLevel == FORCE_LEVEL_2 ) {
		...
		if ( gripEnt->client->ps.forceGripMoveInterval < level.time ) {
			gripEnt->client->ps.velocity.z = 30;
		...
		if ( (level.time - gripEnt->client->ps.fd.forceGripStarted) > 3000 && !self->client->ps.fd.forceGripDamageDebounceTime ) { //if we managed to lift him into the air for 2 seconds, give him a crack
			...
			if ( gripEnt->client->ps.fd.forcePowersActive & (1 << FP_GRIP) ) { //choking, so don't let him keep gripping himself
	...
	if ( gripLevel == FORCE_LEVEL_3 ) {
		...
		if ( gripEnt->client->ps.forceGripMoveInterval < level.time ) {
			...
			AngleVectors( &self->client->ps.viewangles, &fwd, NULL, NULL );
			fwd_o.x = self->client->ps.origin.x + fwd.x * 128;
			fwd_o.y = self->client->ps.origin.y + fwd.y * 128;
			fwd_o.z = self->client->ps.origin.z + fwd.z * 128;
			fwd_o.z += 16;
			VectorSubtract( &fwd_o, &start_o, &nvel );

			nvLen = VectorLength( &nvel );

			if ( nvLen < 16 ) { //within x units of desired spot
				VectorNormalize( &nvel );
				gripEnt->client->ps.velocity.x = nvel.x * 8;
				gripEnt->client->ps.velocity.y = nvel.y * 8;
				gripEnt->client->ps.velocity.z = nvel.z * 8;
			}
			else if ( nvLen < 64 ) {
				VectorNormalize( &nvel );
				gripEnt->client->ps.velocity.x = nvel.x * 128;
				gripEnt->client->ps.velocity.y = nvel.y * 128;
				gripEnt->client->ps.velocity.z = nvel.z * 128;
			}
			else if ( nvLen < 128 ) {
				VectorNormalize( &nvel );
				gripEnt->client->ps.velocity.x = nvel.x * 256;
				gripEnt->client->ps.velocity.y = nvel.y * 256;
				gripEnt->client->ps.velocity.z = nvel.z * 256;
			}
			else if ( nvLen < 200 ) {
				VectorNormalize( &nvel );
				gripEnt->client->ps.velocity.x = nvel.x * 512;
				gripEnt->client->ps.velocity.y = nvel.y * 512;
				gripEnt->client->ps.velocity.z = nvel.z * 512;
			}
			else {
				VectorNormalize( &nvel );
				gripEnt->client->ps.velocity.x = nvel.x * 700;
				gripEnt->client->ps.velocity.y = nvel.y * 700;
				gripEnt->client->ps.velocity.z = nvel.z * 700;
			}
...
#define FORCE_DEBOUNCE_TIME 50 // sv_fps 20 = 50msec frametime
...
static void WP_ForcePowerRun( gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd ) {
	...
	case FP_HEAL:
		if ( self->client->ps.fd.forcePowerLevel[FP_HEAL] == FORCE_LEVEL_1 ) {
			if ( self->client->ps.velocity.x || self->client->ps.velocity.y || self->client->ps.velocity.z ) {
				WP_ForcePowerStop( self, forcePower );
				break;
			}
		}
		...
		if ( self->health > self->client->ps.stats[STAT_MAX_HEALTH] ) { //rww - we might start out over max_health and we don't want force heal taking us down to 100 or whatever max_health is
			WP_ForcePowerStop( self, forcePower );
			break;
		}
	...
	case FP_GRIP:
		...
		if ( self->client->ps.fd.forcePowerDebounce[FP_PULL] < level.time ) { //This is sort of not ideal. Using the debounce value reserved for pull for this because pull doesn't need it.
		...
		break;
	case FP_LEVITATION:
		if ( self->client->ps.groundEntityNum != ENTITYNUM_NONE && !self->client->ps.fd.forceJumpZStart ) {//done with jump
			WP_ForcePowerStop( self, forcePower );
		}
		break;
	case FP_RAGE:
	...
	case FP_DRAIN:
		...
		if ( self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1 ) {//higher than level 1
			if ( (cmd->buttons & BUTTON_FORCE_DRAIN) || ((cmd->buttons & BUTTON_FORCEPOWER) && self->client->ps.fd.forcePowerSelected == FP_DRAIN) ) {//holding it keeps it going
		...
		break;
	case FP_LIGHTNING:
		if ( self->client->ps.forceHandExtend != HANDEXTEND_FORCE_HOLD ) { //Animation for hand extend doesn't end with hand out, so we have to limit lightning intervals by animation intervals (once hand starts to go in in animation, lightning should stop)
			WP_ForcePowerStop( self, forcePower );
			break;
		}

		if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1 ) {//higher than level 1
			if ( (cmd->buttons & BUTTON_FORCE_LIGHTNING) || ((cmd->buttons & BUTTON_FORCEPOWER) && self->client->ps.fd.forcePowerSelected == FP_LIGHTNING) ) {//holding it keeps it going
	...
	case FP_TELEPATHY:
		...
		if ( self->client->holdingObjectiveItem >= MAX_CLIENTS
			&& self->client->holdingObjectiveItem < ENTITYNUM_WORLD
			&& g_entities[self->client->holdingObjectiveItem].genericValue15 ) { //if force hindered can't mindtrick whilst carrying a siege item
			WP_ForcePowerStop( self, FP_TELEPATHY );
		}
		else {
...
int WP_DoSpecificPower( gentity_t *self, usercmd_t *ucmd, forcePowers_t forcepower ) {
	...
	case FP_HEAL:
		...
		if ( self->client->ps.fd.forceButtonNeedRelease ) { //need to release before we can use nonhold powers again
			break;
		}
		ForceHeal( self );
	...
	case FP_PUSH:
		powerSucceeded = 0; //always 0 for nonhold powers
		if ( self->client->ps.fd.forceButtonNeedRelease && !(self->r.svFlags & SVF_BOT) ) { //need to release before we can use nonhold powers again
			break;
		}
...
void FindGenericEnemyIndex( gentity_t *self ) { //Find another client that would be considered a threat.
	int i = 0;
	float tlen;
	gentity_t *ent;
...
void SeekerDroneUpdate( gentity_t *self ) {
	vector3 org, elevated, dir, a, endir;
	gentity_t *en;
	float angle;
	float prefig = 0;
	trace_t tr;
	...
	if ( self->health < 1 ) {
		...
		self->client->ps.eFlags &= ~EF_SEEKERDRONE; //Raz: fixed
		self->client->ps.genericEnemyIndex = -1;
...
void HolocronUpdate( gentity_t *self ) { //keep holocron status updated in holocron mode
	int i = 0;
	int noHRank = 0;
	...
	while ( i < NUM_FORCE_POWERS ) {
		if ( self->client->ps.holocronsCarried[i] ) { //carrying it, make sure we have the power
			self->client->ps.holocronBits |= (1 << i);
			self->client->ps.fd.forcePowersKnown |= (1 << i);
			self->client->ps.fd.forcePowerLevel[i] = FORCE_LEVEL_3;
		}
		else { //otherwise, make sure the power is cleared from us
...
void JediMasterUpdate( gentity_t *self ) { //keep jedi master status updated for JM gametype
	int i = 0;
	...
	while ( i < NUM_FORCE_POWERS ) {
		if ( self->client->ps.isJediMaster ) {
			...
			if ( i == FP_TEAM_HEAL || i == FP_TEAM_FORCE ||
				i == FP_DRAIN || i == FP_ABSORB ) { //team powers are useless in JM, absorb is too because no one else has powers to absorb. Drain is just
			...
			if ( i == FP_TELEPATHY ) { //this decision was made because level 3 mindtrick allows the JM to just hide too much, and no one else has force
...
qboolean G_SpecialRollGetup( gentity_t *self ) { //fixme: currently no knockdown will actually land you on your front... so froll's are pretty useless at the moment.
...
void WP_ForcePowersUpdate( gentity_t *self, usercmd_t *ucmd ) {
	...
	if ( self->client->ps.pm_flags & PMF_FOLLOW ) { //not a "real" game client, it's a spectator following someone
		return;
	}
	if ( self->client->sess.sessionTeam == TEAM_SPECTATOR )
		return;
	...
	*/
	...
	// bad
	if ( self->client->ps.fd.forcePowerSelected >= NUM_FORCE_POWERS )
	...
	if ( ((self->client->sess.selectedFP != self->client->ps.fd.forcePowerSelected) ||
		...
		if ( self->client->sess.updateUITime < level.time ) { //a bit hackish, but we don't want the client to flood with userinfo updates if they rapidly cycle
	...
	if ( self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN &&
		...
		if ( self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN &&
			...
			if ( self->health < 1 || (self->client->ps.eFlags & EF_DEAD) )
				...
				self->client->ps.forceHandExtend = HANDEXTEND_NONE;
			else { //hmm.. ok.. no more getting up on your own, you've gotta push something, unless..
				if ( (level.time - self->client->ps.forceHandExtendTime) > 4000 ) { //4 seconds elapsed, I guess they're too dumb to push something to get up!
					if ( self->client->pers.cmd.upmove &&
						self->client->ps.fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 ) { //force getup
						G_PreDefSound( &self->client->ps.origin, PDSOUND_FORCEJUMP );
						self->client->ps.forceDodgeAnim = 2;
						self->client->ps.forceHandExtendTime = level.time + 500;
	...
	if ( self && self->client && (BG_HasYsalamiri( level.gametype, &self->client->ps ) ||
		self->client->ps.fd.forceDeactivateAll || self->client->tempSpectate >= level.time) ) { //has ysalamiri.. or we want to forcefully stop all his active powers
	...
	else { //otherwise just do a check through them all to see if they need to be stopped for any reason.
		for ( i = 0; i < NUM_FORCE_POWERS; i++ ) {
			if ( (self->client->ps.fd.forcePowersActive & (1 << i)) && i != FP_LEVITATION &&
	...
	if ( self->client->ps.powerups[PW_FORCE_ENLIGHTENED_LIGHT] || self->client->ps.powerups[PW_FORCE_ENLIGHTENED_DARK] ) { //enlightenment
	...
	if ( self->health <= 0 ) {//if dead, deactivate any active force powers
		for ( i = 0; i < NUM_FORCE_POWERS; i++ ) {
	...
	if ( self->client->ps.fd.forceJumpCharge && self->client->ps.groundEntityNum == ENTITYNUM_NONE && self->client->fjDidJump ) { //this was for the "charge" jump method... I guess
	...
	if ( ucmd->buttons & BUTTON_FORCEGRIP ) { //grip is one of the powers with its own button.. if it's held, call the specific grip power function.
		WP_DoSpecificPower( self, ucmd, FP_GRIP );
	}
	else { //see if we're using it generically.. if not, stop.
		if ( self->client->ps.fd.forcePowersActive & (1 << FP_GRIP) ) {
	...
	if ( ucmd->buttons & BUTTON_FORCE_LIGHTNING ) { //lightning
	...
	if ( ucmd->buttons & BUTTON_FORCE_DRAIN ) { //drain
	...
	if ( (ucmd->buttons & BUTTON_FORCEPOWER) && BG_CanUseFPNow( level.gametype, &self->client->ps, level.time,
		(forcePowers_t)self->client->ps.fd.forcePowerSelected ) )
	{
		...
		&& (self->client->ps.weapon != WP_SABER || !BG_SaberInSpecial( self->client->ps.saberMove )) )
	{
		...
		while ( self->client->ps.fd.forcePowerRegenDebounceTime < level.time ) {
			if ( level.gametype != GT_HOLOCRON || g_maxHolocronCarry.value ) {
				...
				holoregen = 0;
				for ( holo = 0; holo < NUM_FORCE_POWERS; holo++ ) {
...
qboolean Jedi_DodgeEvasion( gentity_t *self, gentity_t *shooter, trace_t *tr, int hitLoc ) {
	...
	if ( self->client->ps.groundEntityNum == ENTITYNUM_NONE ) {//can't dodge in mid-air
		return qfalse;
	}

	if ( self->client->ps.weaponTime > 0 || self->client->ps.forceHandExtend != HANDEXTEND_NONE ) {//in some effect that stops me from moving on my own
		return qfalse;
	}
	...
	if ( g_forceDodge.integer == 2 ) {
		...
		if ( self->client->ps.fd.forcePowerLevel[FP_SEE] < FORCE_LEVEL_3 ) {//more likely to fail on lower force sight level
			return qfalse;
		}
	}
